...
public class FaultExecutionService {
  ...
  @Autowired private FaultExecutionRepository faultExecutionRepository;
  ...
  /** 分页获取故障执行记录 */
  @Transactional(readOnly = true)
  public PageResponse<FaultExecution> getFaultExecutions(
      Long executionId,
      Long topologyNodeId,
      String templateType,
      ...
      int size) {
    logger.debug(
        "Getting fault executions with filters - executionId: {}, topologyNodeId: {}, templateType:"
    ...
    Page<FaultExecution> executionPage =
        faultExecutionRepository.findByConditions(
            executionId, topologyNodeId, templateType, resultStatus, pageable);

    return PageResponse.of(
        executionPage.getContent(), executionPage.getTotalElements(), page, size);
  }
  ...
  public FaultExecution getFaultExecutionById(Long id) {
    ...
    return faultExecutionRepository
        .findById(id)
  ...
  public FaultExecution createFaultExecution(FaultExecution faultExecution) {
    logger.info(
        "Creating new fault execution: executionId={}, topologyNodeId={}, templateType={}",
