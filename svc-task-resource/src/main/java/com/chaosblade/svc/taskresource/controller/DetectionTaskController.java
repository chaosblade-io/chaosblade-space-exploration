...
public class DetectionTaskController {
  ...
  @Autowired private DetectionTaskService detectionTaskService;

  @Autowired private EndpointsProperties endpointsProperties;

  /** 获取检测任务列表 GET /api/detection-tasks */
  ...
  public ApiResponse<PageResponse<DetectionTask>> getDetectionTasks(
    ...
    logger.info(
        "GET /api/detection-tasks - systemId: {}, apiId: {}, faultConfigurationsId: {}, "
    ...
    PageResponse<DetectionTask> tasks =
        detectionTaskService.getDetectionTasks(
            systemId,
            apiId,
            faultConfigurationsId,
            sloId,
  ...
  /** 获取检测任务详情（增强版，聚合） GET /api/detection-tasks/{taskId} */
  @GetMapping("/detection-tasks/{taskId}")
  public ApiResponse<com.chaosblade.svc.taskresource.dto.DetectionTaskDtos.DetectionTaskDetails>
      getDetectionTaskDetails(@PathVariable Long taskId) {
    logger.info("GET /api/detection-tasks/{}", taskId);
    var details = detectionTaskService.getDetectionTaskDetails(taskId);
    return ApiResponse.success(details);
  }

  /** 根据名称获取检测任务详情 GET /api/detection-tasks/by-name/{name} */
  ...
  /** 创建新检测任务 POST /api/detection-tasks */
  @PostMapping("/detection-tasks")
  public ApiResponse<DetectionTask> createDetectionTask(
  ...
  /** 更新检测任务信息 PUT /api/detection-tasks/{taskId} */
  @PutMapping("/detection-tasks/{taskId}")
  public ApiResponse<DetectionTask> updateDetectionTask(
      @PathVariable Long taskId, @Valid @RequestBody DetectionTask detectionTask) {
  ...
  /** 部分更新检测任务信息 PATCH /api/detection-tasks/{taskId} */
  @PatchMapping("/detection-tasks/{taskId}")
  public ApiResponse<DetectionTask> patchDetectionTask(
      @PathVariable Long taskId, @RequestBody DetectionTask detectionTask) {
  ...
  /** 删除检测任务 DELETE /api/detection-tasks/{taskId} */
  ...
  /**
   * 执行检测任务（代理转发） POST /api/detection-tasks/{taskId}/execute 代理到 svc-task-executor:
   * http://localhost:8102/api/tasks/{taskId}/execute 说明：该接口保持请求/响应原样透传，不做业务逻辑处理
   */
  @PostMapping("/detection-tasks/{taskId}/execute")
  public org.springframework.http.ResponseEntity<String> executeDetectionTask(
    ...
    try {
      ...
      headers.forEach(
          (k, v) -> {
            ...
            if (org.springframework.http.HttpHeaders.TRANSFER_ENCODING.equalsIgnoreCase(key))
              return;
      ...
      logger.info(
          "[PROXY] executor responded: status={} length={}",
          resp.getStatusCode(),
      ...
      resp.getHeaders()
          .forEach(
              (k, vals) -> {
                ...
                if (org.springframework.http.HttpHeaders.TRANSFER_ENCODING.equalsIgnoreCase(key))
                  return;
                if (org.springframework.http.HttpHeaders.CONTENT_LENGTH.equalsIgnoreCase(key))
                  return;
                if (org.springframework.http.HttpHeaders.CONNECTION.equalsIgnoreCase(key)) return;
                if ("Keep-Alive".equalsIgnoreCase(key)) return;
                if ("Proxy-Connection".equalsIgnoreCase(key)) return;
      ...
      return org.springframework.http.ResponseEntity.status(resp.getStatusCode())
          .headers(outH)
          .body(resp.getBody());
    } catch (Exception ex) {
      ...
      return org.springframework.http.ResponseEntity.status(502)
          ...
          .body(
              "{\"success\":false,\"error\":{\"code\":\"EXECUTOR_PROXY_FAILED\",\"message\":\""
  ...
  /** 取消检测任务 POST /api/detection-tasks/{taskId}/cancel */
  ...
  /** 获取活跃任务列表（未归档） GET /api/detection-tasks/active */
  ...
  /** 获取已归档任务列表 GET /api/detection-tasks/archived */
  ...
  /** 任务执行历史（分页） GET /api/detection-tasks/{taskId}/executions */
  @GetMapping("/detection-tasks/{taskId}/executions")
  public ApiResponse<PageResponse<com.chaosblade.svc.taskresource.entity.TaskExecution>>
      getExecutions(
          @PathVariable Long taskId,
          @RequestParam(value = "page", defaultValue = "1") @Min(1) int page,
          @RequestParam(value = "size", defaultValue = "20") @Min(1) int size) {
  ...
  /** 分页查询所有任务执行记录（支持筛选） GET /api/task-executions 或 /api/executions */
  @GetMapping({"/task-executions", "/executions"})
  public ApiResponse<
          PageResponse<com.chaosblade.svc.taskresource.dto.DetectionTaskDtos.TaskExecutionView>>
      listExecutions(
          ...
          @RequestParam(value = "size", defaultValue = "20") @Min(1) int size) {
    logger.info(
        "GET /api/task-executions - taskId={}, status={}, namespace={}, startDate={}, endDate={},"
            + " page={}, size={}",
        taskId,
        status,
        namespace,
        ...
        size);
    var resp =
        detectionTaskService.getExecutions(
            taskId, status, namespace, startDate, endDate, page, size);
    return ApiResponse.success(resp);
  }

  /**
   * 获取任务执行详情（增强版）- 新路径，去除 taskId GET /api/task-executions/{executionId} 同时提供
   * /api/executions/{executionId} 的别名路径
   */
  @GetMapping({"/task-executions/{executionId}", "/executions/{executionId}"})
  public ApiResponse<com.chaosblade.svc.taskresource.dto.ExecutionDetailsDto>
      getExecutionDetailsNew(@PathVariable Long executionId) {
    ...
    try {
      if (details != null) {
        ...
        if (details.testCases != null) {
          java.util.List<com.chaosblade.svc.taskresource.dto.ExecutionDetailsDto.TestCaseItem>
  ...
  /** 获取任务执行详情（增强版）- 旧路径，保留一段时间用于兼容 GET /api/detection-tasks/{taskId}/executions/{executionId} */
  @Deprecated
  @GetMapping("/detection-tasks/{taskId}/executions/{executionId}")
  public ApiResponse<com.chaosblade.svc.taskresource.dto.ExecutionDetailsDto>
      getExecutionDetailsDeprecated(@PathVariable Long taskId, @PathVariable Long executionId) {
