...
public interface SystemRepository extends JpaRepository<System, Long> {
  ...
  /** 根据名称和所有者查询系统 */
  @Query(
      "SELECT s FROM System s WHERE (:name IS NULL OR s.name LIKE %:name%) AND (:owner IS NULL OR"
          + " s.owner = :owner)")
  Page<System> findByNameAndOwner(
      @Param("name") String name, @Param("owner") String owner, Pageable pageable);
}
