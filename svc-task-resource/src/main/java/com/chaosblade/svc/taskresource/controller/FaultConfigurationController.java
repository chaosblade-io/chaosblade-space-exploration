...
public class FaultConfigurationController {
    ...
    public ApiResponse<PageResponse<FaultConfiguration>> getFaultConfigurations(
        ...
        logger.info("GET /api/fault-configs - faultTypeId: {}, configId: {}, nodeId: {}, page: {}, size: {}", 
                   faultTypeId, configId, nodeId, page, size);
        
        PageResponse<FaultConfiguration> configs = faultConfigurationService.getFaultConfigurations(
                faultTypeId, configId, nodeId, page, size);
    ...
    /**
     * 根据ID获取故障配置详情
     * GET /api/fault-configs/{configId}
     */
    ...
    public ApiResponse<FaultConfiguration> createFaultConfiguration(@Valid @RequestBody FaultConfiguration faultConfiguration) {
        logger.info("POST /api/fault-configs - configId: {}, nodeId: {}, faultTypeId: {}", 
                   faultConfiguration.getConfigId(), faultConfiguration.getNodeId(), faultConfiguration.getFaultTypeId());
        
        FaultConfiguration createdConfig = faultConfigurationService.createFaultConfiguration(faultConfiguration);
        return ApiResponse.success(createdConfig);
    }
    
    /**
     * 删除故障配置
     * DELETE /api/fault-configs/{configId}
     */
