...
public class DetectionTaskService {
  ...
  @Autowired
  private com.chaosblade.svc.taskresource.repository.TestResultRepository testResultRepository;

  @Autowired
  private com.chaosblade.svc.taskresource.repository.TaskExecutionLogRepository
      taskExecutionLogRepository;

  @Autowired
  private com.chaosblade.svc.taskresource.repository.TaskConclusionRepository
      taskConclusionRepository;

  @Autowired private com.chaosblade.svc.taskresource.config.LlmProperties llmProperties;

  /** 获取检测任务列表 */
  @Transactional(readOnly = true)
  public PageResponse<DetectionTask> getDetectionTasks(
      Long systemId,
      Long apiId,
      ...
      int size) {
    logger.debug(
        "Getting detection tasks with filters - systemId: {}, apiId: {}, faultConfigurationsId: {},"
            + " sloId: {}, createdBy: {}, name: {}, startDate: {}, endDate: {}, page: {}, size: {}",
        systemId,
        apiId,
        faultConfigurationsId,
        sloId,
        createdBy,
        name,
        startDate,
    ...
    Page<DetectionTask> taskPage =
        detectionTaskRepository.findByConditions(
            faultConfigurationsId,
            systemId,
            apiId,
            sloId,
  ...
  public DetectionTask getDetectionTaskById(Long taskId) {
    ...
    return detectionTaskRepository
        .findById(taskId)
  ...
  public DetectionTaskDtos.DetectionTaskDetails getDetectionTaskDetails(Long taskId) {
    ...
    DetectionTaskDtos.DetectionTaskDetails details =
        new DetectionTaskDtos.DetectionTaskDetails(task);
    // 系统信息
    details.sys = systemRepository.findById(task.getSystemId()).orElse(null);
    // API 请求定义：优先根据 apiId 选取最新创建的 HttpReqDef（若有多个版本）
    details.apiDefinition =
        (task.getApiId() != null)
    ...
    // 故障配置：按 taskId
    details.faultConfigs =
        faultConfigRepository
            .findByConditions(null, null, taskId, PageRequest.of(0, Integer.MAX_VALUE))
            .getContent();
    // SLO：按 taskId
    details.taskSlos =
        taskSloRepository
            .findByConditions(null, null, null, taskId, null, PageRequest.of(0, Integer.MAX_VALUE))
            .getContent();
    // 执行状态：取最新一条
    details.latestExecutionStatus =
        taskExecutionRepository
            .findTop1ByTaskIdOrderByStartedAtDesc(taskId)
  ...
  public DetectionTask createDetectionTask(DetectionTask detectionTask) {
    ...
    if (!faultConfigurationService.existsById(detectionTask.getFaultConfigurationsId())) {
      throw new BusinessException(
          "FAULT_CONFIG_NOT_FOUND", "故障配置不存在: " + detectionTask.getFaultConfigurationsId());
    }
    ...
    if (detectionTaskRepository.existsByName(detectionTask.getName())) {
      throw new BusinessException(
          "DETECTION_TASK_NAME_EXISTS", "检测任务名称已存在: " + detectionTask.getName());
    }
  ...
  public DetectionTask updateDetectionTask(Long taskId, DetectionTask taskUpdate) {
    ...
    if (taskUpdate.getName() != null && !taskUpdate.getName().equals(existingTask.getName())) {
      if (detectionTaskRepository.existsByName(taskUpdate.getName())) {
        throw new BusinessException(
            "DETECTION_TASK_NAME_EXISTS", "检测任务名称已存在: " + taskUpdate.getName());
      }
    ...
    if (taskUpdate.getFaultConfigurationsId() != null) {
      if (!faultConfigurationService.existsById(taskUpdate.getFaultConfigurationsId())) {
        throw new BusinessException(
            "FAULT_CONFIG_NOT_FOUND", "故障配置不存在: " + taskUpdate.getFaultConfigurationsId());
      }
  ...
  public PageResponse<DetectionTask> getArchivedTasks(int page, int size) {
    ...
    Page<DetectionTask> taskPage =
        detectionTaskRepository.findArchivedTasksWithPagination(pageable);
  ...
  public PageResponse<TaskExecution> getTaskExecutions(Long taskId, int page, int size) {
    ...
    Page<TaskExecution> executions =
        taskExecutionRepository.findByTaskIdOrderByStartedAtDesc(taskId, pageable);
    return PageResponse.of(executions.getContent(), executions.getTotalElements(), page, size);
  }

  /** 获取任务执行详情（增强版）- 旧签名（保留一段时间以兼容） 注意：此方法可能需要写入LLM总结，因此不能设置为只读事务 */
  @Deprecated
  @Transactional
  public ExecutionDetailsDto getExecutionDetails(Long taskId, Long executionId) {
    logger.warn(
        "[DEPRECATED] getExecutionDetails called with taskId={}, executionId={}",
        taskId,
        executionId);
    return getExecutionDetailsByExecutionId(executionId);
  }

  /** 获取任务执行详情（根据 executionId） 注意：此方法可能需要写入LLM总结，因此不能设置为只读事务 */
  @Transactional
  public ExecutionDetailsDto getExecutionDetailsByExecutionId(Long executionId) {
    ...
    TaskExecution exec =
        taskExecutionRepository
            ...
            .orElseThrow(
                () -> new BusinessException("TASK_EXECUTION_NOT_FOUND", "执行记录不存在: " + executionId));
    ...
    logger.info(
        "[ExecDetails] execId={}, taskId={}, status={}, namespace={}, startedAt={}, finishedAt={},"
            + " durationSec={}, apiId={}",
        exec.getId(),
        exec.getTaskId(),
        exec.getStatus(),
    ...
    // 执行日志：读取 executor 侧落库的 task_execution_log
    {
      ...
      for (var lg : logs) {
        ...
        e.ts = lg.getTs();
        e.level =
            switch (java.util.Objects.requireNonNullElse(lg.getLevel(), 1)) {
              case 0 -> "DEBUG";
              case 1 -> "INFO";
              case 2 -> "WARN";
              case 3 -> "ERROR";
    ...
    logger.info(
        "[ExecDetails] cases.size={}, testResults.size={}",
        (cases == null ? 0 : cases.size()),
        (trList == null ? 0 : trList.size()));

    java.util.Map<Long, com.chaosblade.svc.taskresource.entity.TestResult> trMap =
    ...
    if (cases != null) {
      com.fasterxml.jackson.databind.ObjectMapper om2 =
      ...
      for (TestCase tc : cases) {
        ...
        if (tr == null) {
          try {
            java.util.List<?> arr =
                (tc.getFaultsJson() == null || tc.getFaultsJson().isBlank())
                    ? java.util.List.of()
                    : om2.readValue(tc.getFaultsJson(), java.util.List.class);
            String caseIdStr;
            ...
            logger.debug(
                "[ExecDetails] fallback-mapping: caseIdStr='{}', legacyId={}, found={} (tcId={})",
                caseIdStr,
                legacyId,
                (tr != null),
                tc.getId());
        ...
        // 构造指标对象
        ExecutionDetailsDto.TestCaseItem.MetricResult mP50 =
            new ExecutionDetailsDto.TestCaseItem.MetricResult();
        ExecutionDetailsDto.TestCaseItem.MetricResult mP95 =
            new ExecutionDetailsDto.TestCaseItem.MetricResult();
        ExecutionDetailsDto.TestCaseItem.MetricResult mP99 =
            new ExecutionDetailsDto.TestCaseItem.MetricResult();
        ExecutionDetailsDto.TestCaseItem.MetricResult mErr =
            new ExecutionDetailsDto.TestCaseItem.MetricResult();
        if (tr != null) {
        ...
        logger.debug(
            "[ExecDetails] caseId={}, matchedMetrics={}, p50={}, p95={}, p99={}, errRate={}",
            tc.getId(),
            (tr != null),
            (tr != null ? tr.getP50() : null),
            (tr != null ? tr.getP95() : null),
            (tr != null ? tr.getP99() : null),
            (tr != null ? tr.getErrRate() : null));
    ...
    dto.testCases = items;
    {
      ...
      if (items != null) {
        for (var tci : items) {
          if ((tci.p50 != null && tci.p50.value != null)
              || (tci.p95 != null && tci.p95.value != null)
              || (tci.p99 != null && tci.p99.value != null)
              || (tci.errRate != null && tci.errRate.value != null)) cntWith++;
        }
      }
      logger.info(
          "[ExecDetails] testCases.size={}, withMetrics={}, withoutMetrics={}",
          total,
          cntWith,
          (total - cntWith));
    }
    ...
    logger.info(
        "[ExecDetails] svc2types.size={}, faultInjections.size={}, services={}",
        svc2types.size(),
        fi.size(),
        svc2types.keySet());
    ...
    if ("DONE".equalsIgnoreCase(exec.getStatus())) {
      ...
      var sloPage =
          taskSloRepository.findByConditions(
              null,
              null,
              null,
              task.getId(),
      ...
      for (var slo : sloPage.getContent()) {
        if (slo.getP95() != null)
          sloP95 = (sloP95 == null) ? slo.getP95() : Math.min(sloP95, slo.getP95());
        if (slo.getP99() != null)
          sloP99 = (sloP99 == null) ? slo.getP99() : Math.min(sloP99, slo.getP99());
        if (slo.getErrRate() != null)
          sloErr = (sloErr == null) ? slo.getErrRate() : Math.min(sloErr, slo.getErrRate());
      }
      logger.info(
          "[ExecDetails] SLO thresholds: p95={}, p99={}, errRate%={}", sloP95, sloP99, sloErr);
      ...
      String worstDesc = null;
      String worstSloViol = null;

      com.fasterxml.jackson.databind.ObjectMapper omFail =
          new com.fasterxml.jackson.databind.ObjectMapper();
      ...
      for (var it : dto.testCases) {
        ...
        if (it.p50 != null && it.p50.value instanceof Integer && sloP95 != null) {
          p50Ok = ((Integer) it.p50.value) <= sloP95;
          it.p50.meetsSlo = p50Ok;
        } else if (it.p50 != null) {
          it.p50.meetsSlo = null;
        }
        if (it.p95 != null && it.p95.value instanceof Integer && sloP95 != null) {
          p95Ok = ((Integer) it.p95.value) <= sloP95;
          it.p95.meetsSlo = p95Ok;
        }
        if (it.p99 != null && it.p99.value instanceof Integer && sloP99 != null) {
          p99Ok = ((Integer) it.p99.value) <= sloP99;
          it.p99.meetsSlo = p99Ok;
        }
        if (it.errRate != null && it.errRate.value != null && sloErr != null) {
          ...
          java.math.BigDecimal percent =
              (val.compareTo(java.math.BigDecimal.ONE) <= 0)
                  ? val.multiply(new java.math.BigDecimal("100"))
                  : val; // 统一转为百分比
          java.math.BigDecimal sloB = new java.math.BigDecimal(String.valueOf(sloErr));
          errOk = percent.compareTo(sloB) <= 0;
          it.errRate.meetsSlo = errOk;
        }
        boolean hasAnyThreshold = (sloP95 != null) || (sloP99 != null) || (sloErr != null);
        boolean meetsAll = true;
        if (sloP95 != null) {
          if (it.p95 == null || it.p95.value == null) meetsAll = false;
          else meetsAll &= Boolean.TRUE.equals(p95Ok);
        }
        if (sloP99 != null) {
          if (it.p99 == null || it.p99.value == null) meetsAll = false;
          else meetsAll &= Boolean.TRUE.equals(p99Ok);
        }
        if (sloErr != null) {
          if (it.errRate == null || it.errRate.value == null) meetsAll = false;
          else meetsAll &= Boolean.TRUE.equals(errOk);
        }
        // 若无任何阈值，默认视为成功
        ...
        if (meetsAll) succ++;
          ...
          try {
            if (it.faultsJson != null && !it.faultsJson.isBlank()) {
              ...
              // 偏离度
              double dev = 0.0;
              if (sloP95 != null && it.p95 != null && it.p95.value instanceof Integer)
                dev += Math.max(0, ((Integer) it.p95.value) - sloP95);
              if (sloP99 != null && it.p99 != null && it.p99.value instanceof Integer)
                dev += Math.max(0, ((Integer) it.p99.value) - sloP99);
              ...
              if (dev > maxDeviation) {
                ...
                if (sloP95 != null
                    && it.p95 != null
                    && it.p95.value instanceof Integer
                    && ((Integer) it.p95.value) > sloP95) viol.add("p95>");
                if (sloP99 != null
                    && it.p99 != null
                    && it.p99.value instanceof Integer
                    && ((Integer) it.p99.value) > sloP99) viol.add("p99>");
                if (sloErr != null && it.errRate != null && it.errRate.value != null) {
                  java.math.BigDecimal val = (java.math.BigDecimal) it.errRate.value;
                  java.math.BigDecimal percent =
                      (val.compareTo(java.math.BigDecimal.ONE) <= 0)
                          ? val.multiply(new java.math.BigDecimal("100"))
                          : val;
                  if (percent.compareTo(new java.math.BigDecimal(String.valueOf(sloErr))) > 0)
                    viol.add("errRate>");
                }
            ...
            logger.warn(
                "[ExecDetails] parse faultsJson failed: caseId={}, err={}", it.id, exx.toString());
          }
        }
      }
      ...
      // 3) 主要失败类型与最受影响服务
      String mainFailType = null;
      int typeMax = 0;
      for (var e : failTypeCnt.entrySet()) {
        if (e.getValue() > typeMax) {
          typeMax = e.getValue();
          mainFailType = e.getKey();
        }
      }
      String worstService = null;
      int svcMax = 0;
      for (var e : failSvcCnt.entrySet()) {
        if (e.getValue() > svcMax) {
          svcMax = e.getValue();
          worstService = e.getKey();
        }
      }
      ...
      try {
        ...
        if (dto.llmSummary == null) {
          // 拓扑数据
          var topoOpt =
              apiTopologyRepository.findBySystemIdAndApiId(task.getSystemId(), task.getApiId());
          ...
          String prompt =
              "请基于以下数据，按固定模板，输出<=300字中文总结：\\n"
                  ...
                  + fail
                  + ", 成功率="
                  + String.format(java.util.Locale.ROOT, "%.1f", succRate)
                  + "%。\\n"
                  + "主要失败类型="
                  + String.valueOf(mainFailType)
                  + "；最受影响服务="
                  + String.valueOf(worstService)
                  + "。\\n"
                  + "典型失败: 用例#"
                  + String.valueOf(worstId)
                  + "，描述="
                  + String.valueOf(worstDesc)
                  + "，指标="
                  + String.valueOf(worstChange)
                  + "，违规="
                  + String.valueOf(worstViol)
                  + "。\\n"
                  + "拓扑节点数="
                  + nodes.size()
                  + "，边数="
            ...
            try {
              ...
              if (llmResp != null) {
                com.fasterxml.jackson.databind.JsonNode root =
                    new com.fasterxml.jackson.databind.ObjectMapper().readTree(llmResp);
          ...
          if (summary == null || summary.isBlank()) {
            logger.warn(
                "[ExecDetails] LLM summary generation failed, resp={}, ex={}",
                llmResp,
                (lastEx == null ? null : lastEx.toString()));
          } else {
            // 保存LLM总结到数据库
    ...
    logger.info(
        "[ExecDetails] realtime: totalCases={}, completedCases={}, totalSvcs={}, completedSvcs={},"
            + " testingSvcs={}",
        r.totalTestCases,
        r.completedTestCases,
        r.totalServices,
        r.completedServices,
        r.testingServices);

    r.testingServices =
        ("INJECTING_AND_REPLAYING".equalsIgnoreCase(exec.getStatus()))
            ? Math.max(0, r.totalServices - r.completedServices)
            : 0;
    dto.realtime = r;
  ...
  /** 分页查询所有执行记录（可筛选） */
  @Transactional(readOnly = true)
  public PageResponse<DetectionTaskDtos.TaskExecutionView> getExecutions(
      Long taskId,
      String status,
      String namespace,
      LocalDateTime startDate,
      ...
      int size) {
    logger.debug(
        "Query executions: taskId={}, status={}, namespace={}, startDate={}, endDate={}, page={},"
            + " size={}",
        taskId,
        status,
        namespace,
        ...
        size);
    Pageable pageable = PageRequest.of(page - 1, size);
    Page<TaskExecution> p =
        taskExecutionRepository.findByConditions(
            taskId, status, namespace, startDate, endDate, pageable);
