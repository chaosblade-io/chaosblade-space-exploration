...
public class FaultExecutionController {
    ...
    @Autowired
    private FaultExecutionService faultExecutionService;
    
    /**
     * 获取故障执行列表
     * GET /api/fault-executions
     */
    ...
    public ApiResponse<PageResponse<FaultExecution>> getFaultExecutions(
            ...
            @RequestParam(value = "resultStatus", required = false) FaultExecution.ResultStatus resultStatus,
            @RequestParam(value = "page", defaultValue = "1") @Min(1) int page,
            @RequestParam(value = "size", defaultValue = "20") @Min(1) int size) {
        
        logger.info("GET /api/fault-executions - executionId: {}, topologyNodeId: {}, templateType: {}, resultStatus: {}, page: {}, size: {}", 
                   executionId, topologyNodeId, templateType, resultStatus, page, size);
        
        PageResponse<FaultExecution> executions = faultExecutionService.getFaultExecutions(
                executionId, topologyNodeId, templateType, resultStatus, page, size);
    ...
    /**
     * 根据ID获取故障执行详情
     * GET /api/fault-executions/{executionId}
     */
    ...
    /**
     * 创建新的故障执行记录
     * POST /api/fault-executions
     */
    @PostMapping("/fault-executions")
    public ApiResponse<FaultExecution> createFaultExecution(@Valid @RequestBody FaultExecution faultExecution) {
        logger.info("POST /api/fault-executions - executionId: {}, topologyNodeId: {}, templateType: {}", 
    ...
    /**
     * 删除故障执行记录
     * DELETE /api/fault-executions/{executionId}
     */
