...
public class FaultTypeService {
  ...
  public PageResponse<FaultType> getFaultTypes(
      String category, String severityLevel, String status, String name, int page, int size) {
    logger.debug(
        "Getting fault types with filters - category: {}, enabled: {}, faultCode: {}, name: {},"
            + " page: {}, size: {}",
        category,
        severityLevel,
        status,
    ...
    Pageable pageable =
        PageRequest.of(page - 1, size, Sort.by(Sort.Direction.ASC, "category", "name"));
    Page<FaultType> faultTypePage =
        faultTypeRepository.findByConditions(category, enabled, faultCode, name, pageable);

    return PageResponse.of(
        faultTypePage.getContent(), faultTypePage.getTotalElements(), page, size);
  }
  ...
  public FaultType getFaultTypeById(Long faultTypeId) {
    ...
    return faultTypeRepository
        .findById(faultTypeId)
        .orElseThrow(
            () -> new BusinessException("FAULT_TYPE_NOT_FOUND", "故障类型不存在: " + faultTypeId));
  }
  ...
  public FaultType updateFaultType(Long faultTypeId, FaultType faultTypeUpdate) {
    ...
    // 检查名称是否与其他故障类型冲突
    if (faultTypeUpdate.getName() != null
        && !faultTypeUpdate.getName().equals(existingFaultType.getName())) {
      if (faultTypeRepository.existsByName(faultTypeUpdate.getName())) {
        throw new BusinessException(
            "FAULT_TYPE_NAME_EXISTS", "故障类型名称已存在: " + faultTypeUpdate.getName());
      }
