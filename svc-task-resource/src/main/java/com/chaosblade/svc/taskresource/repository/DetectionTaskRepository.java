...
public interface DetectionTaskRepository extends JpaRepository<DetectionTask, Long> {
     ...
     * 根据多个条件查询检测任务
     */
    @Query("SELECT dt FROM DetectionTask dt WHERE " +
    ...
    Page<DetectionTask> findByConditions(@Param("faultConfigurationsId") Long faultConfigurationsId,
     ...
     * 统计指定时间范围内的任务数量
     */
    @Query("SELECT COUNT(dt) FROM DetectionTask dt WHERE dt.createdAt BETWEEN :startDate AND :endDate")
