...
public class HttpReqDefService {
  ...
  @Autowired private HttpReqDefRepository repository;

  private static final Pattern URL_TEMPLATE_PATTERN = Pattern.compile("^/.*");

  @Transactional(readOnly = true)
  public PageResponse<HttpReqDef> pageQuery(
      String name, HttpReqDef.HttpMethod method, Long apiId, int page, int size) {
  ...
  public HttpReqDef create(HttpReqDef def) {
    ...
    if (repository.existsByCode(def.getCode())) {
      throw new BusinessException("HTTP_REQ_DEF_CODE_EXISTS", "请求定义编码已存在: " + def.getCode());
    }
  ...
  private void validate(HttpReqDef def, boolean creating) {
    ...
    if (!URL_TEMPLATE_PATTERN.matcher(def.getUrlTemplate()).matches()) {
      throw new BusinessException("URL_TEMPLATE_INVALID", "URL 模板需以 / 开头，如 /api/{name}/list");
    }
