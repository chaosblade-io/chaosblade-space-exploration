...
public class ApiService {
  ...
  public PageResponse<Api> getApisBySystemId(
      Long systemId, String method, String path, String tags, String version, int page, int size) {
    logger.debug(
        "Getting APIs for system: {}, method: {}, path: {}, tags: {}, version: {}, page: {}, size:"
            + " {}",
        systemId,
        method,
        path,
    ...
    Pageable pageable =
        PageRequest.of(page - 1, size, Sort.by(Sort.Direction.ASC, "path", "method"));
    Page<Api> apiPage =
        apiRepository.findByConditions(systemId, method, path, tags, version, pageable);
  ...
  public Api getApiBySystemIdAndOperationId(Long systemId, String operationId) {
    ...
    return apiRepository
        .findBySystemIdAndOperationId(systemId, operationId)
        .orElseThrow(
            () ->
  ...
  public Api updateApi(Long apiId, Api apiUpdate) {
    ...
    if (apiUpdate.getOperationId() != null
        && !apiUpdate.getOperationId().equals(existingApi.getOperationId())) {
      if (apiRepository.existsBySystemIdAndOperationId(
          existingApi.getSystemId(), apiUpdate.getOperationId())) {
        throw new BusinessException(
            "API_OPERATION_ID_EXISTS",
            String.format(
                "操作ID在系统内已存在: %s (系统ID: %d)",
