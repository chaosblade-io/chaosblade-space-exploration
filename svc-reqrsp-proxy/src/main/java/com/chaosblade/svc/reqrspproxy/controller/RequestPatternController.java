...
public class RequestPatternController {
  ...
  @Autowired private RequestPatternService requestPatternService;
  @Autowired private ReplayService replayService;

  /**
   * 获取请求模式 POST /api/request-patterns/analyze
   *
   * <p>请求参数示例： { "reqDefId": 1, "namespace": "train-ticket", "serviceList": [
   * "ts-preserve-service", "ts-security-service", "ts-order-service", "ts-order-other-service",
   * "ts-contacts-service", "ts-travel-service", "ts-ticketinfo-service", "ts-basic-service",
   * "ts-station-service", "ts-route-service", "ts-train-service", "ts-price-service",
   * "ts-seat-service", "ts-config-service", "ts-food-service", "ts-consign-service",
   * "ts-consign-price-service", "ts-user-service" ] }
   *
   * <p>响应格式： { "code": "200", "message": "success", "data": { "taskId":
   * "task_1693123456789_abc12345", "status": "PROCESSING", "message": "任务已启动", "reqDefId": 1,
   * "namespace": "train-ticket", "serviceList": [...], "requestPatterns": [ { "serviceName":
   * "ts-preserve-service", "requestMode": [ { "method": "POST", "url":
   * "/api/v1/preserveservice/preserve" } ] } ], "startTime": "2023-08-27 14:30:00", "endTime":
   * "2023-08-27 14:35:00", "totalRecordedRequests": 25, "analyzedServices": 18, "recordingId":
   * "rec_20230827_143000_xyz" } }
   */
  @PostMapping("/analyze")
  public ApiResponse<RequestPatternResponse> analyzeRequestPatterns(
    ...
    logger.info(
        "POST /api/request-patterns/analyze - reqDefId: {}, execution_id: {}, namespace: {},"
            + " services: {}",
        request.getReqDefId(),
        request.getExecutionId(),
        request.getNamespace(),
        request.getServiceList() == null ? 0 : request.getServiceList().size());
  ...
  /**
   * 获取任务状态 GET /api/request-patterns/tasks/{taskId}
   *
   * <p>响应示例： { "code": "200", "message": "success", "data": { "taskId":
   * "task_1693123456789_abc12345", "status": "ROLLING_UPDATE", "message":
   * "等待服务滚动更新完成，应用Envoy配置中...", "reqDefId": 1, "namespace": "train-ticket", "serviceList": [...],
   * "startTime": "2023-08-27 14:30:00", "totalRecordedRequests": 0, "analyzedServices": 0,
   * "recordingId": "rec_20230827_143000_xyz" } }
   *
   * <p>状态说明： - INITIALIZING: 初始化阶段 - APPLYING_RULES: 规则应用阶段 - ROLLING_UPDATE: 滚动更新阶段 -
   * TRIGGERING_REQUESTS: 请求发起阶段 - COLLECTING_DATA: 数据收集阶段 - ANALYZING_PATTERNS: 模式分析阶段 - COMPLETED:
   * 完成阶段 - FAILED: 失败阶段
   */
  ...
  /**
   * 手动触发请求发起 POST /api/request-patterns/tasks/{taskId}/trigger
   *
   * <p>用于在滚动更新完成后手动触发请求发起
   */
  ...
  /**
   * 获取请求模式记录详情 GET /api/request-patterns/tasks/{taskId}/details
   *
   * <p>获取任务的详细信息，包括录制数据和分析结果
   */
  ...
  /**
   * 停止请求模式记录 POST /api/request-patterns/tasks/{taskId}/stop
   *
   * <p>手动停止正在进行的请求模式记录任务
   *
   * <p>响应示例： { "code": "200", "message": "success", "data": "录制已停止，正在分析数据..." }
   */
  ...
  /** 健康检查接口 GET /api/request-patterns/health */
  @GetMapping("/health")
  ...
  /** Redis 数据统计接口 GET /api/request-patterns/redis-stats */
  ...
  /** 请求重放 POST /api/request-patterns/replay */
  @PostMapping("/replay")
  public ApiResponse<List<ReplayResult>> replay(@Valid @RequestBody ReplayRequest request) {
    logger.info(
        "POST /api/request-patterns/replay - execution_id: {}, namespace: {}, service_name: {}",
        request.getExecutionId(),
        request.getNamespace(),
