...
public class RecordingStateService {
  ...
  public void saveState(RecordingState state) {
    try {
      ...
      if (savedJson != null) {
        logger.info(
            "Successfully saved and verified recording state for {}", state.getRecordingId());
      } else {
      ...
      logger.error(
          "Failed to save recording state for {}: {}", state.getRecordingId(), e.getMessage(), e);
  ...
  public boolean exists(String recordingId) {
    try {
      ...
      logger.error(
          "Failed to check recording existence for {}: {}", recordingId, e.getMessage(), e);
      return false;
    }
  }
  ...
  public void updateStatus(String recordingId, RecordingState.RecordingStatus status) {
    try {
      if (!exists(recordingId)) {
        logger.warn(
            "Skip updateStatus: recording {} not found (target status={})", recordingId, status);
        return;
      }
  ...
  public void setError(String recordingId, String errorMessage) {
    try {
      if (!exists(recordingId)) {
        logger.warn(
            "Skip setError: recording {} not found, message={} (cleaned?)",
            recordingId,
            errorMessage);
        return;
      }
  ...
  public Map<String, Object> getRedisStats() {
    ...
    try {
      ...
      if (recordingStateKeys != null && !recordingStateKeys.isEmpty()) {
        stats.put(
            "recentRecordings", recordingStateKeys.stream().limit(10).collect(Collectors.toList()));
      }
