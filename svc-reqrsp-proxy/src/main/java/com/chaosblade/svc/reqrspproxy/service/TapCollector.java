...
public class TapCollector {
    ...
    public TapCollector(RecordingSettings settings) {
        ...
        logger.info("TapCollector executor initialized with concurrency {}", settings.getTapConcurrent());
    }
    ...
    public CompletableFuture<Integer> collectOnce(String recordingId, RecordingState state) {
        ...
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                logger.error("Failed to collect data for recording {}: {}", recordingId, e.getMessage(), e);
    ...
    private int collectFromPod(String recordingId, RecordingState state, Pod pod) {
        ...
        try {
            ...
            java.util.concurrent.atomic.AtomicInteger collected = new java.util.concurrent.atomic.AtomicInteger(0);
            tapFiles.parallelStream().forEach(fileName -> {
                try {
                    if (!isRecordingActive(recordingId)) {
                        return; // 已停止，取消处理
                    }
                    ...
                    String content = readTapFileWithRetry(recordingId, state.getNamespace(), podName, fileName, settings.getTapReadRetryMaxRetries(), settings.getTapReadRetrySleepMillis());
                    ...
                    logger.warn("Failed to process tap file {} in pod {}: {}", fileName, podName, ex.getMessage());
                }
            });
     ...
     */
    private String readTapFileWithRetry(String recordingId, String namespace, String podName, String fileName, int maxRetries, long sleepMillis) {
    ...
    public int exportRawTapFiles(String recordingId, RecordingState state, Path outDir) {
        ...
        try {
            ...
            List<Pod> pods = getPodsByService(state.getNamespace(), state.getServiceName());
            pods.parallelStream().forEach(pod -> {
                String podName = pod.getMetadata().getName();
                if (!isEnvoyContainerReady(pod)) {
                ...
                List<String> tapFiles = listTapFiles(state.getNamespace(), podName);
                tapFiles.parallelStream().forEach(filePath -> {
                    try {
                        if (!isRecordingActive(recordingId)) return;
                        String content = readTapFileWithRetry(recordingId, state.getNamespace(), podName, filePath, 5, 300);
                        if (content == null || content.trim().isEmpty()) return;
                        String base = filePath;
                        int idx = base.lastIndexOf('/');
                        if (idx >= 0) base = base.substring(idx + 1);
                        String fileName = String.format("%s__%s__%s", state.getServiceName(), podName, base);
                        ...
                        logger.warn("Failed exporting raw tap from pod {} file {}: {}", podName, filePath, ex.getMessage());
                    }
                });
            });
        } catch (Exception e) {
    ...
    private List<String> listTapFiles(String namespace, String podName) {
        try {
            String command = String.format("ls -1 %s/*.json 2>/dev/null || true", recordingConfig.getEnvoy().getTapDir());
            String output = execCommandWithRetry(namespace, podName, "envoy", command,
                    settings.getTapReadRetryMaxRetries(), settings.getTapReadRetrySleepMillis());
    ...
    private String readTapFile(String namespace, String podName, String fileName) {
        try {
            ...
            return execCommandWithRetry(namespace, podName, "envoy", command,
                    settings.getTapReadRetryMaxRetries(), settings.getTapReadRetrySleepMillis());

        } catch (Exception e) {
            logger.error("Failed to read tap file {} from pod {}: {}", fileName, podName, e.getMessage(), e);
            return null;
        }
    }
     ...
     */
    private String execCommand(String namespace, String podName, String containerName, String command) {
        ByteArrayOutputStream out = new ByteArrayOutputStream();
        ByteArrayOutputStream err = new ByteArrayOutputStream();

        try (ExecWatch watch = k8s.pods()
            ...
            // 特殊处理容器不存在的情况
            if (e.getMessage() != null && e.getMessage().contains("container " + containerName + " not found")) {
                logger.debug("Container {} not found in pod {}, this is expected during startup/shutdown", containerName, podName);
                return "";
            }
    ...
    private String execCommandWithRetry(String namespace, String podName, String containerName, String command,
                                        int maxRetries, long sleepMillis) throws InterruptedException {
        for (int attempt = 1; attempt <= Math.max(1, maxRetries); attempt++) {
            try {
                ...
                boolean handshake = msg.contains("WebSocketHandshakeException") || msg.contains("Exec Failure");
     ...
     * 解析 tap 内容
     */
    private RecordedEntry parseTapContent(String recordingId, RecordingState state, Pod pod, String content) {
        try {
    ...
    private boolean isEnvoyContainerReady(Pod pod) {
        ...
        if (!"Running".equals(pod.getStatus().getPhase())) {
            logger.debug("Pod {} is not in Running phase: {}",
                    pod.getMetadata().getName(), pod.getStatus().getPhase());
            return false;
        }

        // 检查是否有 Envoy 容器
        boolean hasEnvoyContainer = pod.getSpec().getContainers().stream()
        ...
        if (pod.getStatus().getContainerStatuses() != null) {
            return pod.getStatus().getContainerStatuses().stream()
                    ...
                    .findFirst()
                    .map(status -> Boolean.TRUE.equals(status.getReady()) && Boolean.TRUE.equals(status.getStarted()))
                    .orElse(false);
        }
