...
public class K8sTapManager {
    ...
    private final java.util.concurrent.ConcurrentHashMap<String, java.util.concurrent.ScheduledFuture<?>> autoStopTasks = new java.util.concurrent.ConcurrentHashMap<>();
    ...
    public void injectOrUpdateSidecar(String namespace, String deploymentName, String cmName) {
        logger.info("Injecting/updating Envoy sidecar in deployment {} in namespace {}", deploymentName, namespace);
        ...
        for (int attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                k8s.apps().deployments().inNamespace(namespace).withName(deploymentName).edit(deployment -> {
                    PodSpec spec = deployment.getSpec().getTemplate().getSpec();

                    // 确保 volumes 存在
                    ensureVolume(spec, "envoy-config", new VolumeBuilder()
                    ...
                    ensureVolume(spec, "envoy-taps", new VolumeBuilder()
                            .withName("envoy-taps")
                    ...
                    int envoyIndex = IntStream.range(0, finalContainers.size())
                ...
                logger.info("Envoy sidecar injected/updated successfully in deployment {} (attempt {})", deploymentName, attempt);
                ...
                if (e.getCode() == 409 && attempt < maxRetries) {
                    ...
                    logger.warn("Deployment {} conflict detected during sidecar injection (attempt {}), retrying in {}ms: {}",
                               deploymentName, attempt, retryDelay, e.getMessage());
                    ...
                    // 其他错误或重试次数用完
                    logger.error("Failed to inject/update sidecar in deployment {} after {} attempts: {}",
                                deploymentName, attempt, e.getMessage(), e);
    ...
    public void redirectServiceToEnvoy(String namespace, String serviceName, int targetPort) {
        logger.info("Redirecting service {} to port {} in namespace {}", serviceName, targetPort, namespace);
        ...
        try {
            ...
            logger.error("Failed to redirect service {} to port {}: {}", serviceName, targetPort, e.getMessage(), e);
    ...
    public void waitRolloutReady(String namespace, String deploymentName) {
        ...
        try {
            ...
            logger.error("Failed to wait for deployment {} readiness: {}", deploymentName, e.getMessage(), e);
    ...
    public void removeSidecarOrDisableTap(String namespace, String deploymentName) {
        logger.info("Removing Envoy sidecar from deployment {} in namespace {}", deploymentName, namespace);
        ...
        try {
            ...
            logger.error("Failed to remove sidecar from deployment {}: {}", deploymentName, e.getMessage(), e);
    ...
    public void scheduleAutoStop(String recordingId, Integer durationSec, Runnable stopCallback) {
        ...
        java.util.concurrent.ScheduledFuture<?> future = taskScheduler.schedule(() -> {
            ...
            try {
                ...
                logger.error("Failed to auto-stop recording {}: {}", recordingId, e.getMessage(), e);
            } finally {
    ...
    private void ensureVolume(PodSpec spec, String name, Volume volume) {
        ...
        int index = IntStream.range(0, finalVolumes.size())
    ...
    public void restoreServiceToOriginal(String namespace, String serviceName, int originalPort) {
        logger.info("Restoring service {} to original port {} in namespace {}", serviceName, originalPort, namespace);
    ...
    public void triggerRollingUpdateForConfigChange(String namespace, String deploymentName) {
        logger.info("Triggering rolling update for config change in deployment {} in namespace {}", deploymentName, namespace);
        triggerRollingUpdate(namespace, deploymentName);
    }

    /**
     * 触发 Deployment 的滚动更新
     * 通过添加/更新 annotation 来强制 Pod 重启
     * 包含重试机制处理并发冲突
     */
    private void triggerRollingUpdate(String namespace, String deploymentName) {
        logger.info("Triggering rolling update for deployment {} in namespace {}", deploymentName, namespace);
        ...
        for (int attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                k8s.apps().deployments().inNamespace(namespace).withName(deploymentName).edit(deployment -> {
                    ...
                    // 添加重启注解
                    deployment.getSpec().getTemplate().getMetadata().getAnnotations().put(restartAnnotation, timestamp);
                ...
                logger.info("Rolling update triggered successfully for deployment {} (attempt {})", deploymentName, attempt);
                ...
                if (e.getCode() == 409 && attempt < maxRetries) {
                    // 409 Conflict - 资源版本冲突，重试
                    logger.warn("Deployment {} conflict detected (attempt {}), retrying in {}ms: {}",
                               deploymentName, attempt, retryDelay, e.getMessage());
                    ...
                    // 其他错误或重试次数用完
                    logger.error("Failed to trigger rolling update for deployment {} after {} attempts: {}",
                                deploymentName, attempt, e.getMessage(), e);
    ...
    private void waitForRollingUpdateToStart(String namespace, String deploymentName) {
        ...
        try {
            ...
            for (int i = 0; i < 30; i++) {
                Deployment deployment = k8s.apps().deployments()
                    .inNamespace(namespace)
                    .withName(deploymentName)
                    .get();
                ...
                if (deployment != null && deployment.getStatus() != null) {
                    ...
                    if (updatedReplicas != null && replicas != null && updatedReplicas > 0) {
                        logger.info("Rolling update started for deployment {}, updated replicas: {}/{}",
                                   deploymentName, updatedReplicas, replicas);
                        return;
                    }
                }
            ...
            logger.warn("Rolling update may not have started within 30 seconds for deployment {}", deploymentName);
