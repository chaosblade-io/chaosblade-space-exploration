...
/**
 * 基于外部模板的 Envoy Tap 配置渲染器
 * 使用 Mustache 模板引擎处理动态内容
 */
@Component
public class TemplateBasedTapConfigRenderer {
    
    private static final Logger logger = LoggerFactory.getLogger(TemplateBasedTapConfigRenderer.class);
    ...
    private Map<String, Object> buildTemplateContext(int appPort, List<RecordingRule> rules) {
        ...
        logger.debug("Template context: adminPort={}, envoyPort={}, appPort={}, rules={}, useSingleRule={}",
                context.get("adminPort"), context.get("envoyPort"), appPort, ruleList.size(), context.get("useSingleRule"));
