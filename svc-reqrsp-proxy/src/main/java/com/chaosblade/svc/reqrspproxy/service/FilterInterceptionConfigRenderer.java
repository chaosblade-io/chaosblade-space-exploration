...
/**
 * 基于 envoy/envoy-tap-filter.tmpl.yaml 的拦截配置渲染器
 * 生成带有 direct_response 与 baggage 条件匹配的 Envoy 配置
 */
@Component
public class FilterInterceptionConfigRenderer {
    private static final Logger logger = LoggerFactory.getLogger(FilterInterceptionConfigRenderer.class);
    ...
    public void initTemplate() {
        try {
            ...
            logger.error("Failed to load Envoy filter template from {}: {}", TEMPLATE_PATH, e.getMessage(), e);
     ...
     */
    public String render(int appPort, List<FixtureUpsertRequest.FixtureItem> items, boolean tapEnabled, String tapFilePrefix) {
        ...
        if (tapEnabled) {
            ctx.put("tapFilePrefix", tapFilePrefix != null ? tapFilePrefix : (recordingConfig.getEnvoy().getTapDir() + "/fi-"));
        }

        List<Map<String, Object>> intercepts = items.stream()
                .map(this::toInterceptMap)
    ...
    private Map<String, Object> toInterceptMap(FixtureUpsertRequest.FixtureItem item) {
        ...
        if (statusIn == null || statusIn < 200 || statusIn >= 600) {
            statusOut = 500;
            logger.warn("Invalid respStatus {} for interception rule (service={}, path={}), defaulting to 500", statusIn, item.getServiceName(), item.getPath());
        } else {
            statusOut = statusIn;
        }
        ...
        if (body.isEmpty()) {
            body = "{\"status\":0,\"msg\":\"Service temporarily unavailable due to interception rule\",\"data\":null}";
            logger.warn("Empty respBody for interception rule (service={}, path={}), defaulting to fallback JSON", item.getServiceName(), item.getPath());
        }
    ...
    public boolean isTemplateLoaded() { return template != null; }
}
