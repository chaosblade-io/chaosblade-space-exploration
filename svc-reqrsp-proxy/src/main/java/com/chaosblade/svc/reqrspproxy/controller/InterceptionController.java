 ...
 * 智能拦截控制器
 *
 * <p>核心功能： 1. 智能检测目标服务的录制状态 2. 基于现有配置添加拦截功能 3. 支持独立的拦截模式
 *
 * <p>API 路径：/api/interceptions
 */
@RestController
...
public class InterceptionController {
  ...
  @Autowired private InterceptionService interceptionService;

  @Autowired private RecordingStateService stateService;

  /**
   * 智能添加拦截规则 POST /api/interceptions/start
   *
   * <p>自动检测目标服务状态： - 如果已在录制，基于现有配置添加拦截 - 如果未在录制，启动仅拦截模式
   */
  @PostMapping("/start")
  public ApiResponse<InterceptionResponse> addInterception(
      @Valid @RequestBody AddInterceptionRequest request) {
    logger.info(
        "POST /api/interceptions/start - namespace: {}, serviceName: {}, rules: {}",
  ...
  /** 移除拦截规则 DELETE /api/interceptions/session/{sessionId}/rules */
  @DeleteMapping("/session/{sessionId}/rules")
  public ApiResponse<InterceptionResponse> removeInterceptionRules(
      @PathVariable String sessionId, @RequestBody List<InterceptionRule> rules) {
    logger.info(
        "DELETE /api/interceptions/session/{}/rules - removing {} rules", sessionId, rules.size());
  ...
  /** 获取拦截状态 GET /api/interceptions/session/{sessionId}/status */
  @GetMapping("/session/{sessionId}/status")
  public ApiResponse<InterceptionStatusResponse> getInterceptionStatus(
  ...
  /** 检查服务的录制状态 GET /api/interceptions/check/{namespace}/{serviceName} */
  ...
  public ApiResponse<ServiceRecordingStatusResponse> checkServiceRecordingStatus(
    ...
    try {
      ...
      RecordingState existingRecording =
          activeRecordings.stream()
              .filter(
                  state ->
      ...
      logger.error(
          "Failed to check service recording status for {}/{}: {}",
          namespace,
          serviceName,
          e.getMessage(),
          e);
      return ApiResponse.error(
          "500", "Failed to check service recording status: " + e.getMessage());
    }
  }

  /** 获取所有活跃的拦截会话 GET /api/interceptions/active */
  @GetMapping("/active")
  public ApiResponse<List<InterceptionSummary>> getActiveInterceptions() {
    ...
    try {
      ...
      List<InterceptionSummary> summaries =
          activeStates.stream()
  ...
  /** 诊断拦截问题 GET /api/interceptions/session/{sessionId}/diagnose */
  ...
  public ApiResponse<Map<String, Object>> diagnoseInterception(@PathVariable String sessionId) {
    ...
    try {
      ...
      testCommands.put(
          "checkConfigMap",
          String.format(
              "kubectl get configmap %s -n %s", state.getConfigMapName(), state.getNamespace()));
      testCommands.put(
          "checkService",
          String.format(
              "kubectl get service %s -n %s -o yaml",
              state.getServiceName(), state.getNamespace()));
      testCommands.put(
          "checkPods",
          String.format(
              "kubectl get pods -n %s | grep %s", state.getNamespace(), state.getServiceName()));

      if (!state.getInterceptionRules().isEmpty()) {
        testCommands.put(
            "testInterception",
            String.format(
                "curl -X %s http://%s:12031%s -H 'Content-Type: application/json' -d"
                    + " '{\"test\":\"data\"}' -v",
