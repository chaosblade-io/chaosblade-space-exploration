 ...
 * 混合配置渲染器 - 同时支持录制和拦截功能
 *
 * <p>核心设计： 1. Tap 过滤器在 Router 之前，确保所有请求都能被录制 2. 拦截路由优先级高于转发路由 3. 被拦截的请求也会被 Tap 记录（包含模拟响应）
 */
@Component
public class HybridConfigRenderer {
  ...
  /** 渲染混合配置（录制 + 拦截） */
  public String renderHybridConfig(
      int appPort, List<RecordingRule> recordingRules, List<InterceptionRule> interceptionRules) {

    logger.debug(
        "Rendering hybrid config: appPort={}, recordingRules={}, interceptionRules={}",
        appPort,
        recordingRules.size(),
        interceptionRules.size());

    // 1. 生成拦截路由块
    String interceptRoutes =
        interceptionRules.stream().map(this::renderInterceptRoute).collect(Collectors.joining());
  ...
  private String renderInterceptRoute(InterceptionRule rule) {
    ...
    String responseHeaders =
        rule.getMockResponse().getHeaders().entrySet().stream()
            .map(
                entry ->
                    String.format(
                        "                    - header: { key: \"%s\", value: \"%s\" }",
                        escapeYamlString(entry.getKey()), escapeYamlString(entry.getValue())))
  ...
  private String generateTapMatchConfig(List<RecordingRule> recordingRules) {
    ...
    if (recordingRules.size() == 1) {
      ...
      return String.format(
          SINGLE_TAP_MATCH_TEMPLATE,
          escapeYamlString(rule.getPath()),
          rule.getMethod().toUpperCase());
    } else {
      // 多个规则时使用 or_match
      String tapMatches =
          recordingRules.stream().map(this::renderTapMatch).collect(Collectors.joining());
  ...
  private String renderTapMatch(RecordingRule rule) {
    return String.format(
        TAP_MATCH_TEMPLATE, escapeYamlString(rule.getPath()), rule.getMethod().toUpperCase());
  }

  /** YAML 字符串转义 对于 inline_string，我们需要更安全的转义方式 */
