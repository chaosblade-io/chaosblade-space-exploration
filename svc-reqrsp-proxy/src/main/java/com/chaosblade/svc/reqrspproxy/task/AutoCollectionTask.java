        ...
        try {
            ...
            for (String stateKey : stateKeys) {
                ...
                if (!stateService.exists(recordingId)) {
                    logger.debug("Skip auto-collect, state missing for {}. Clean up key {}", recordingId, stateKey);
                ...
                try {
                    ...
                    if (state.getStatus() == RecordingState.RecordingStatus.RECORDING) {
                        ...
                        if (startedAt != null) {
                            long secondsSinceStart = java.time.Duration.between(startedAt, java.time.LocalDateTime.now()).getSeconds();
                            if (secondsSinceStart < 20) {
                                logger.debug("Skip auto-collect for {} ({}s since RECORDING < 20s)", recordingId, secondsSinceStart);
                                continue;
                            }
                        }

                        tapCollector.collectOnce(recordingId, state)
                                .thenAccept(count -> {
                                    if (count > 0) {
                                        logger.debug("Auto-collected {} entries for recording {}", count, recordingId);
                                    }
                                })
                                .exceptionally(throwable -> {
                                    logger.warn("Auto-collection failed for recording {}: {}", recordingId, throwable.getMessage());
                                    return null;
                                });
                        collectedCount++;
                        ...
                        logger.info("Cleaning up finished recording {}: status={}", recordingId, state.getStatus());
            ...
            if (collectedCount > 0 || cleanedCount > 0) {
                logger.info("Auto collection task completed: collected from {} recordings, cleaned up {} finished recordings",
                           collectedCount, cleanedCount);
            }
