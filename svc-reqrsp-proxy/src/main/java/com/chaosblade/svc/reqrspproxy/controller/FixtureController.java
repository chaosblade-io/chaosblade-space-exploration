...
public class FixtureController {
  ...
  @Autowired private FixtureInterceptionService fixtureService;

  /** 批量更新fixture规则 */
  @PostMapping("/upsert")
  public ResponseEntity<FixtureUpsertResponse> upsertFixtures(
      @Valid @RequestBody FixtureUpsertRequest request, HttpServletRequest httpRequest) {
    logger.info(
        "POST /api/fixtures/upsert - recordId: {}, namespace: {}, items: {}, ttlSec: {}",
        request.getRecordId(),
    ...
    try {
      ...
      logger.info(
          "Fixture upsert completed successfully: upserted={}, expiresAt={}",
          response.getUpserted(),
  ...
  public ResponseEntity<Map<String, Object>> deleteByRecordId(@PathVariable String recordId) {
    ...
    try {
      ...
      return ResponseEntity.ok(Map.of("deleted", true, "recordId", recordId));
    } catch (Exception e) {
