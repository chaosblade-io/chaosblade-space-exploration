...
public class TaskStateManager {
    ...
    // 内存中的任务状态缓存
    private final ConcurrentHashMap<String, RequestPatternResponse> taskCache = new ConcurrentHashMap<>();
    ...
    public RequestPatternResponse getTaskState(String taskId) {
        try {
            ...
            if (json != null) {
                RequestPatternResponse response = objectMapper.readValue(json, RequestPatternResponse.class);
     ...
     * 更新任务进度信息
     */
    public void updateTaskProgress(String taskId, Integer totalRecordedRequests, Integer analyzedServices) {
        RequestPatternResponse response = getTaskState(taskId);
        if (response != null) {
