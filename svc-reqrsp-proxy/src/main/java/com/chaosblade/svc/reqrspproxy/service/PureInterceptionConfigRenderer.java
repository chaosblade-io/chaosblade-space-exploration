...
/** 纯拦截配置渲染器 生成只包含拦截功能的 Envoy 配置，不包含录制（Tap）功能 */
@Component
public class PureInterceptionConfigRenderer {

  private static final Logger logger =
      LoggerFactory.getLogger(PureInterceptionConfigRenderer.class);

  /** 渲染纯拦截 Envoy 配置 */
  public String renderPureInterceptionConfig(
      int appPort, List<InterceptionRule> interceptionRules) {
  ...
  private String renderResponseHeaders(InterceptionRule rule) {
    ...
    if (rule.getMockResponse().getContentType() != null) {
      headers.append(
          String.format(
              RESPONSE_HEADER_TEMPLATE, "content-type", rule.getMockResponse().getContentType()));
    }

    // 添加自定义头
    if (rule.getMockResponse().getHeaders() != null) {
      rule.getMockResponse()
          .getHeaders()
          .forEach(
              (key, value) -> {
                headers.append(
                    String.format(
                        RESPONSE_HEADER_TEMPLATE, escapeYamlString(key), escapeYamlString(value)));
              });
    }
