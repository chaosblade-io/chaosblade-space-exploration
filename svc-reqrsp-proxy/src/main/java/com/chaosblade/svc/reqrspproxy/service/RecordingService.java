...
public class RecordingService {
  ...
  @Autowired private TemplateBasedTapConfigRenderer tapRenderer;
  ...
  public RecordingResponse start(StartRecordingRequest request) {
    ...
    logger.info(
        "Starting recording {} for service {}/{}",
        recordingId,
    ...
    try {
      ...
      RecordingState state =
          new RecordingState(recordingId, request.getNamespace(), request.getServiceName());
      ...
      if (appPort == null) {
        ...
        if (appPort == null) {
          throw new RuntimeException(
              "Cannot determine application port for service: " + request.getServiceName());
        }
      }
        ...
        tapManager.scheduleAutoStop(
            ...
            () -> {
              try {
                ...
                logger.error(
                    "Auto-stop failed for recording {}: {}", recordingId, e.getMessage(), e);
  ...
  public RecordingResponse stop(String recordingId) {
    ...
    try {
      ...
      try {
        tapManager.cancelAutoStop(recordingId);
      ...
      try {
        if (state.getConfigMapName() != null) {
          ...
          logger.info(
              "Deleted ConfigMap {} in namespace {}",
              state.getConfigMapName(),
              state.getNamespace());
        }
      } catch (Exception e) {
      ...
      try {
        ...
        logger.warn(
            "Skip setting error for {} because state missing or already cleaned: {}",
            recordingId,
            ex.getMessage());
      }
  ...
  public RecordingStatusResponse getStatus(String recordingId) {
    ...
    try {
      ...
      RecordingStatusResponse response =
          new RecordingStatusResponse(recordingId, state.getStatus().name());
      ...
      // 获取部署和服务状态
      try {
        response.setDeploymentStatus(
            getDeploymentStatus(state.getNamespace(), state.getDeploymentName()));
  ...
  public List<RecordedEntry> getEntries(String recordingId, int offset, int limit) {
    logger.debug(
        "Getting entries for recording {} (offset={}, limit={})", recordingId, offset, limit);
