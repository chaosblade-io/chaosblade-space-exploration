...
public class RequestPatternAnalyzer {
     ...
     * 分析录制的请求数据，提取服务请求模式
     */
    public List<ServiceRequestPattern> analyzeRequestPatterns(List<RecordedEntry> recordedEntries,
            List<String> targetServices) {

        logger.info("Analyzing request patterns for {} recorded entries and {} target services",
                recordedEntries.size(), targetServices.size());

        // 按服务名分组请求
        Map<String, List<RequestInfo>> serviceRequestMap = groupRequestsByService(recordedEntries, targetServices);
     ...
     * 按服务名分组请求
     */
    private Map<String, List<RequestInfo>> groupRequestsByService(List<RecordedEntry> recordedEntries,
            List<String> targetServices) {
            ...
            try {
                ...
                if (requestInfo != null) {
                    ...
                    if (serviceName != null) {
                        ...
                        logger.debug("Skip entry without determinable service. url={}, path={}", requestInfo.getUrl(), requestInfo.getPath());
                    }
                }
            } catch (Exception e) {
        ...
        try {
            ...
            if (respHeaders != null) {
                ...
                if (StringUtils.hasText(t)) {
                    try { respTime = Long.parseLong(t.trim()); } catch (NumberFormatException ignore) {}
                }
            }

            if (StringUtils.hasText(method) && StringUtils.hasText(url)) {
                return new RequestInfo(method.toUpperCase(), url, path, reqHeaders, reqBody,
                        respHeaders, respBody, respStatus, respTime);
            }
    ...
    private String determineTargetService(RequestInfo requestInfo, List<String> targetServices) {
        ...
        if (StringUtils.hasText(path)) {
            for (String serviceName : targetServices) {
                ...
                if (path.startsWith(needle1) || path.startsWith(needle2) || path.contains("/" + serviceName + "/")) {
                    return serviceName;
                }
            }
        }
     ...
     * 为单个服务提取请求模式
     */
    private ServiceRequestPattern extractServicePattern(String serviceName, List<RequestInfo> requests) {
        // 去重并排序请求模式（保持原有逻辑：仅按 method + url；url 使用完整URL，保留查询串）
        Set<ServiceRequestPattern.RequestMode> uniqueModes = requests.stream()
                .map(req -> new ServiceRequestPattern.RequestMode(
                        req.getMethod(),
                        req.getUrl(),
                        req.getHeaders(),
                        req.getBody(),
                ...
                .collect(Collectors.toCollection(() -> new TreeSet<>((a, b) -> {
                    ...
                    return methodCompare != 0 ? methodCompare : a.getUrl().compareTo(b.getUrl());
                })));
    ...
    private static class RequestInfo {
        ...
        public RequestInfo(String method, String url, String path,
                           Map<String, String> headers, String body,
                           Map<String, String> respHeaders, String respBody,
                           Integer respStatus, Long respTime) {
            this.method = method;
            this.url = url;
            this.path = path;
        ...
        public Map<String, String> getRespHeaders() { return respHeaders; }
        public String getRespBody() { return respBody; }
        public Integer getRespStatus() { return respStatus; }
        public Long getRespTime() { return respTime; }
    }
}
