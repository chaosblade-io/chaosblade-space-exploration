 ...
 * 混合服务 - 同时支持录制和拦截功能
 *
 * <p>核心功能： 1. 管理录制和拦截的混合配置 2. 确保拦截的请求也能被正确录制 3. 支持动态添加/移除拦截规则
 */
@Service
public class HybridService {
  ...
  public HybridResponse startHybrid(StartHybridRequest request) {
    logger.info(
        "Starting hybrid mode: namespace={}, serviceName={}, recordingRules={},"
            + " interceptionRules={}",
        request.getNamespace(),
        request.getServiceName(),
        request.getRecordingRules().size(),
    ...
    try {
      ...
      List<RecordingRule> allRecordingRules =
          mergeRecordingRules(request.getRecordingRules(), request.getInterceptionRules());

      // 4. 生成混合 Envoy 配置
      String envoyYaml =
          hybridRenderer.renderHybridConfig(
              appPort, allRecordingRules, request.getInterceptionRules());
      ...
      RecordingState state =
          createHybridState(hybridId, request, configMapName, deploymentName, appPort);
      stateService.saveState(state);

      logger.info("Hybrid mode started successfully: hybridId={}", hybridId);
      return new HybridResponse(
          hybridId, "ACTIVE", allRecordingRules.size(), request.getInterceptionRules().size());
  ...
  public HybridResponse addInterceptionRules(String hybridId, List<InterceptionRule> newRules) {
    logger.info(
        "Adding interception rules to hybrid session: hybridId={}, newRules={}",
        hybridId,
        newRules.size());
    ...
    try {
      ...
      List<RecordingRule> allRecordingRules =
          mergeRecordingRules(state.getRules(), allInterceptionRules);

      // 4. 重新生成配置
      String envoyYaml =
          hybridRenderer.renderHybridConfig(
              state.getAppPortOriginal(), allRecordingRules, allInterceptionRules);
  ...
  private List<RecordingRule> mergeRecordingRules(
      List<RecordingRule> recordingRules, List<InterceptionRule> interceptionRules) {
  ...
  /** 创建混合状态对象 */
  private RecordingState createHybridState(
      String hybridId,
      StartHybridRequest request,
  ...
  /** 移除拦截规则 */
  public HybridResponse removeInterceptionRules(
      String hybridId, List<InterceptionRule> rulesToRemove) {
    logger.info(
        "Removing interception rules from hybrid session: hybridId={}, rulesToRemove={}",
        hybridId,
        rulesToRemove.size());
    ...
    try {
      ...
      return new HybridResponse(
          hybridId, state.getStatus().name(), state.getRules().size(), currentRules.size());
  ...
  public HybridResponse updateInterceptionRules(String hybridId, List<InterceptionRule> newRules) {
    logger.info(
        "Updating interception rules for hybrid session: hybridId={}, newRules={}",
        hybridId,
        newRules.size());
    ...
    try {
      ...
      String envoyYaml =
          hybridRenderer.renderHybridConfig(
              state.getAppPortOriginal(), allRecordingRules, newRules);
      ...
      return new HybridResponse(
          hybridId, state.getStatus().name(), allRecordingRules.size(), newRules.size());
