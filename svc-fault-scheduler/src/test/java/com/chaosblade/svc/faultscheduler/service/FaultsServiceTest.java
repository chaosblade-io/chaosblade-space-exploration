...
class FaultsServiceTest {
  ...
  @Mock private ChaosBladeApi bladeApi;

  @Mock private FaultRedisRepo repo;

  @Mock private ThreadPoolTaskScheduler scheduler;

  @Mock private SpecNormalizer normalizer;
  ...
  void testExecuteSuccess() {
    ...
    GenericKubernetesResource mockResource =
  ...
  void testExecuteWithGeneratedName() {
    ...
    verify(scheduler, never())
        .schedule(any(Runnable.class), any(java.util.Date.class)); // TTL = 0，不应该调度
  }
  ...
  void testStatusAndEventsSuccess() {
    ...
    GenericKubernetesResource mockBlade =
        new GenericKubernetesResourceBuilder()
            .withApiVersion("chaosblade.io/v1alpha1")
            .withKind("ChaosBlade")
            .build();
    ...
    List<Map<String, Object>> events =
        List.of(Map.of("type", "Normal", "reason", "Created", "message", "Fault created"));
