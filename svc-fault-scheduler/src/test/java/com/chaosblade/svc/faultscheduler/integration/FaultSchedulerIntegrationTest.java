...
class FaultSchedulerIntegrationTest {
                    ...
                    Map.of(
                        ...
                        "matchers",
                            List.of(
                                ...
                                Map.of(
                                    "name", "container-names", "value", List.of("test-container")),
  ...
  void testExecuteFaultWithInvalidSpec() throws Exception {
    ...
    // 准备无效的测试数据
    Map<String, Object> invalidSpec = Map.of("spec", Map.of("invalid", "spec"));
  ...
  void testExecuteFaultWithEmptyBody() throws Exception {
    ...
    mockMvc
        .perform(post("/api/faults:execute").contentType(MediaType.APPLICATION_JSON).content("{}"))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.success").value(false));
  }
  ...
  void testGetFaultStatusNotFound() throws Exception {
    ...
    mockMvc
        .perform(get("/api/faults/non-existent-fault/status"))
  ...
  void testStopFaultNotFound() throws Exception {
    ...
    mockMvc.perform(delete("/api/faults/non-existent-fault")).andExpect(status().isNotFound());
  }
  ...
  void testCheckFaultExists() throws Exception {
    ...
    mockMvc
        .perform(get("/api/faults/test-fault/exists"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.success").value(true))
  ...
  void testExecuteFaultWithFullCR() throws Exception {
    ...
    Map<String, Object> fullCR =
        Map.of(
            ...
            "ChaosBlade",
            "metadata",
            Map.of("name", "will-be-overridden", "labels", Map.of("test", "integration")),
            "spec",
            Map.of(
                "experiments",
                List.of(
  ...
  void testInvalidHttpMethods() throws Exception {
    ...
    // 测试不支持的 HTTP 方法
    mockMvc.perform(put("/api/faults:execute")).andExpect(status().isMethodNotAllowed());

    mockMvc
        .perform(patch("/api/faults/test-fault/status"))
        .andExpect(status().isMethodNotAllowed());

    mockMvc.perform(post("/api/faults/test-fault")).andExpect(status().isMethodNotAllowed());
  }
  ...
  void testInvalidPathParameters() throws Exception {
    ...
    // 测试空的路径参数
    mockMvc.perform(get("/api/faults//status")).andExpect(status().isNotFound());

    mockMvc.perform(delete("/api/faults/")).andExpect(status().isNotFound());
  }
}
