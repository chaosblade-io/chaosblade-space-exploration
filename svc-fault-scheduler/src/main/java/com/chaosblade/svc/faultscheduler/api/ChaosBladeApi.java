...
/** ChaosBlade API 客户端 封装对 Kubernetes ChaosBlade CRD 的操作 */
@Component
public class ChaosBladeApi {
  ...
  public GenericKubernetesResource create(
      String name, Map<String, String> labels, Map<String, Object> spec) {
    try {
  ...
  private boolean createResourceWithKubectl(String yamlContent) {
    try {
      ...
      java.nio.file.Files.write(
          tempFile, yamlContent.getBytes(java.nio.charset.StandardCharsets.UTF_8));

      try {
        // 构建 kubectl 命令，使用集群内的 service account
        ProcessBuilder pb = new ProcessBuilder("kubectl", "apply", "-f", tempFile.toString());
  ...
  private String buildChaosBladeYaml(
      String name, Map<String, String> labels, Map<String, Object> spec) {
    try {
      ...
      if (experimentsObj instanceof java.util.List) {
        java.util.List<Map<String, Object>> experiments =
            (java.util.List<Map<String, Object>>) experimentsObj;
        ...
        for (Map<String, Object> experiment : experiments) {
          ...
          if (matchersObj instanceof java.util.List) {
            java.util.List<Map<String, Object>> matchers =
                (java.util.List<Map<String, Object>>) matchersObj;
  ...
  public GenericKubernetesResource get(String name) {
    try {
      ...
      GenericKubernetesResource blade = client.genericKubernetesResources(CTX).withName(name).get();
  ...
  public boolean delete(String name) {
    try {
      ...
      List<io.fabric8.kubernetes.api.model.StatusDetails> result =
          client.genericKubernetesResources(CTX).withName(name).delete();
  ...
  public List<Map<String, Object>> eventsForBlade(String bladeName, int limit) {
    ...
    try {
      ...
      try {
        ...
        logger.debug(
            "Failed to retrieve events.k8s.io/v1 events (may not be available): {}",
            e.getMessage());
      }
