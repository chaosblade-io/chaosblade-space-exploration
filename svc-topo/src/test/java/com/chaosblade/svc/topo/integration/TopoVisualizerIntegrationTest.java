...
class TopoVisualizerIntegrationTest {
  ...
  void testCompleteWorkflow() throws Exception {
    // 1. 测试健康检查
    mockMvc
        .perform(get("/api/trace/health"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.status").value("UP"));

    // 2. 测试文件格式查询
    mockMvc
        .perform(get("/api/trace/formats"))
        .andExpect(status().isOk())
    ...
    MockMultipartFile file =
        new MockMultipartFile("file", "test-trace.json", "application/json", traceJson.getBytes());

    MvcResult uploadResult =
        mockMvc
            .perform(multipart("/api/trace/upload").file(file))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.success").value(true))
    ...
    mockMvc
        .perform(
            post("/api/visualization/mermaid/default")
  ...
  void testFileValidation() throws Exception {
    ...
    mockMvc
        .perform(multipart("/api/trace/upload").file(emptyFile))
    ...
    MockMultipartFile invalidFile =
        new MockMultipartFile(
            "file", "invalid.json", "application/json", "invalid json".getBytes());

    mockMvc
        .perform(multipart("/api/trace/upload").file(invalidFile))
  ...
  void testErrorHandling() throws Exception {
    ...
    MockMultipartFile txtFile =
        new MockMultipartFile("file", "test.txt", "text/plain", "not a json file".getBytes());

    mockMvc
        .perform(multipart("/api/trace/upload").file(txtFile))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.success").value(false));
  }
  ...
  void testJsonBasedGeneration() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/trace/generate").contentType(MediaType.APPLICATION_JSON).content(traceJson))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.success").value(true))
        .andExpect(jsonPath("$.topology").exists())
        .andExpect(jsonPath("$.mermaidDiagram").exists());
