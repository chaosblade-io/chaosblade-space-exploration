...
class TopologyAutoRefreshServiceTest {
  ...
  @Mock private JaegerQueryService jaegerQueryService;

  @Mock private TopologyConverterService topologyConverterService;

  @Mock private TraceParserService traceParserService;

  @InjectMocks private TopologyAutoRefreshService autoRefreshService;
  ...
  void testManualRefreshWithValidData() {
    ...
    // 配置 mock 行为
    when(jaegerQueryService.queryTracesByOperation(
            anyString(), anyInt(), anyString(), anyString(), anyLong(), anyLong()))
        .thenReturn(mockTraceData);
    when(topologyConverterService.convertTraceToTopology(mockTraceData)).thenReturn(mockTopology);
    ...
    verify(jaegerQueryService, times(1))
        .queryTracesByOperation(
            eq("localhost"), eq(14250), eq("frontend"), eq("all"), anyLong(), anyLong());
    verify(topologyConverterService, times(1)).convertTraceToTopology(mockTraceData);
    verify(topologyConverterService, times(1)).setCurrentTopology(mockTopology);
  }
  ...
  void testManualRefreshWithEmptyTraceData() {
    ...
    verify(jaegerQueryService, times(1))
        .queryTracesByOperation(
            anyString(), anyInt(), anyString(), anyString(), anyLong(), anyLong());
    verify(topologyConverterService, never()).convertTraceToTopology(any());
    verify(topologyConverterService, never()).setCurrentTopology(any());
  }
