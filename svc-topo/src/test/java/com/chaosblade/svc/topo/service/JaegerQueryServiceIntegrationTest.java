 ...
 *
 * <p>注意：此测试需要运行中的Jaeger实例 可以通过以下命令启动Jaeger： docker run -d --name jaeger -p 16686:16686 -p
 * 16685:16685 jaegertracing/all-in-one:latest
 */
@SpringBootTest
@TestPropertySource(properties = {"logging.level.com.chaosblade.svc.topo.service=DEBUG"})
class JaegerQueryServiceIntegrationTest {
  ...
  void setUp() {
    ...
    // 从JVM系统属性或环境变量获取配置
    jaegerHost =
        System.getProperty("jaeger.host", System.getenv().getOrDefault("JAEGER_HOST", "localhost"));
    jaegerPort =
        Integer.parseInt(
            System.getProperty(
                "jaeger.port", System.getenv().getOrDefault("JAEGER_PORT", "16685")));

    testServiceName =
        System.getProperty(
            "test.service.name", System.getenv().getOrDefault("TEST_SERVICE_NAME", "frontend"));
    testOperationName =
        System.getProperty(
            "test.operation.name", System.getenv().getOrDefault("TEST_OPERATION_NAME", "all"));
    testTraceId =
        System.getProperty(
            "test.trace.id",
  ...
  void testPerformanceWithMultipleQueries() {
    ...
    // 验证性能：10次查询应该在10秒内完成
    assertTrue(duration < 10000, String.format("10次查询花费时间过长: %d ms", duration));
  }
  ...
  void testResourceCleanup() {
    ...
    // 调用shutdown方法
    assertDoesNotThrow(() -> jaegerQueryService.shutdown(), "资源清理不应该抛出异常");
  }
}
