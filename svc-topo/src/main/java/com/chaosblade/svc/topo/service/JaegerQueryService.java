...
public class JaegerQueryService {
   ...
   */
  public TraceData queryTracesByOperation(
      String jaegerHost,
      int port,
      String serviceName,
      ...
      long startTime,
      long endTime) {
    return queryTracesByOperation(
        jaegerHost, port, serviceName, operationName, startTime, endTime, DEFAULT_TRACE_LIMIT);
  }
  ...
  public TraceData queryTracesByOperation(
    ...
    logger.info(
        "开始从Jaeger查询trace数据: host={}, port={}, service={}, operation={}, startTime={}, endTime={},"
            + " limit={}",
        jaegerHost,
        port,
        serviceName,
    ...
    for (int attempt = 1; attempt <= MAX_RETRY_ATTEMPTS; attempt++) {
      try {
        ...
        // 构建查询参数 (将毫秒转换为秒和纳秒)
        Query.TraceQueryParameters queryParameters =
            Query.TraceQueryParameters.newBuilder()
        ...
        // 构建查询请求
        Query.FindTracesRequest request =
            Query.FindTracesRequest.newBuilder().setQuery(queryParameters).build();
        ...
        lastException = e;
        logger.warn(
            "Jaeger gRPC调用失败 (尝试 {}/{}): {}", attempt, MAX_RETRY_ATTEMPTS, e.getStatus(), e);
        ...
        lastException = e;
        logger.warn(
            "查询Jaeger trace数据时发生异常 (尝试 {}/{}): {}", attempt, MAX_RETRY_ATTEMPTS, e.getMessage(), e);
        if (attempt < MAX_RETRY_ATTEMPTS) {
          try {
            Thread.sleep(RETRY_DELAY_MS);
          } catch (InterruptedException ie) {
   ...
   * @throws RuntimeException 当连接或查询失败时
   */
  public TraceData queryTracesByServiceHttp(
      JaegerSource jaegerSource, long startTime, long endTime) {
    // 参数验证
    ...
    logger.info(
        "开始从Jaeger通过HTTP API查询trace数据 (/api/traces): host={}, port={}, service={}, startTime={},"
            + " endTime={}, limit={}",
    ...
    for (int attempt = 1; attempt <= MAX_RETRY_ATTEMPTS; attempt++) {
      try (CloseableHttpClient httpClient = HttpClients.createDefault()) {
        ...
        urlBuilder
            .append("http://")
            .append(jaegerSource.getHost())
            .append(":")
            .append(jaegerSource.getHttpPort())
            .append(jaegerSource.getBasePath() != null ? jaegerSource.getBasePath() : "/api/traces")
            .append("?limit=")
            .append(limit)
            .append("&service=")
            .append(jaegerSource.getEntryService())
            .append("&start=")
            .append(startUs)
            .append("&end=")
            .append(endUs);
        ...
        logger.info(
            "成功从Jaeger通过HTTP API获取trace数据，共{}个trace记录",
            traceData.getData() != null ? traceData.getData().size() : 0);
        return traceData;
        ...
        lastException = e;
        logger.warn(
            "通过HTTP API查询Jaeger trace数据时发生异常 (尝试 {}/{}): {}",
            attempt,
            MAX_RETRY_ATTEMPTS,
            e.getMessage(),
            e);
    ...
    throw new RuntimeException(
        "Failed to query Jaeger via HTTP API after "
            + MAX_RETRY_ATTEMPTS
            + " attempts: "
            + lastException.getMessage(),
        lastException);
   ...
   * @throws RuntimeException 当连接或查询失败时
   */
  public TraceData queryTracesByOperationHttp(
    ...
    int limit = jaegerSource.getLimit() != null ? jaegerSource.getLimit() : DEFAULT_TRACE_LIMIT;
    logger.info(
        "开始从Jaeger通过HTTP API查询trace数据 (/api/traces): host={}, port={}, service={}, operation={},"
  ...
  /** 获取或创建gRPC stub */
  private QueryServiceGrpc.QueryServiceBlockingStub getOrCreateStub(
      ManagedChannel channel, String host, int port) {
    String stubKey = host + ":" + port;

    return stubCache.computeIfAbsent(
        stubKey,
        key -> {
  ...
  /** 验证服务查询参数 */
  private void validateServiceParameters(
      String jaegerHost, int port, String serviceName, long startTime, long endTime) {
    if (jaegerHost == null || jaegerHost.trim().isEmpty()) {
      throw new IllegalArgumentException("Jaeger host cannot be null or empty");
    }
    if (port <= 0 || port > 65535) {
  ...
  private void validateJaegerSource(JaegerSource jaegerSource, long startTime, long endTime) {
    ...
    if (jaegerSource.getHttpPort() == null
        || jaegerSource.getHttpPort() <= 0
        || jaegerSource.getHttpPort() > 65535) {
      throw new IllegalArgumentException("Port must be between 1 and 65535");
    }
  ...
  /** 处理FindTraces响应 */
  private TraceData processFindTracesResponse(
      Iterator<Query.SpansResponseChunk> responseIterator, int limit) {
    TraceData traceData = new TraceData();
    List<TraceData.TraceRecord> traces = new ArrayList<>();
  ...
  private SpanData convertSpan(Model.Span span) {
    ...
    spanData.setStartTime(
        span.getStartTime().getSeconds() * 1_000_000 + span.getStartTime().getNanos() / 1000);
    spanData.setDuration(
        span.getDuration().getSeconds() * 1_000_000 + span.getDuration().getNanos() / 1000);
    ...
    for (Model.Log log : span.getLogsList()) {
      ...
      logEntry.setTimestamp(
          log.getTimestamp().getSeconds() * 1_000_000 + log.getTimestamp().getNanos() / 1000);
  ...
  private Object getValueFromKeyValue(Model.KeyValue keyValue) {
    switch (keyValue.getVType()) {
      ...
      case INT64:
        return keyValue.getVInt64();
      case FLOAT64:
        return keyValue.getVFloat64();
      case BINARY:
        return keyValue.getVBinary().toByteArray();
      default:
        return keyValue.getVStr();
    }
  }
