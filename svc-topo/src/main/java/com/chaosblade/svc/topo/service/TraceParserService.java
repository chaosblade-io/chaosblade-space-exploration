 ...
 * Trace文件解析服务
 *
 * <p>功能： 1. 解析OpenTelemetry trace-*.json文件 2. 提取服务拓扑信息 3. 构建服务调用关系
 */
@Service
public class TraceParserService {
  ...
  public List<ServiceCall> extractServiceCalls(TraceData traceData) {
    ...
    for (TraceData.TraceRecord record : traceData.getData()) {
      ...
      Map<String, String> processToService =
          record.getProcesses().entrySet().stream()
              .collect(
                  Collectors.toMap(
                      Map.Entry::getKey,
      ...
      Map<String, SpanData> spanMap =
          record.getSpans().stream()
              .collect(
                  Collectors.toMap(
                      SpanData::getSpanId, span -> span, (existing, replacement) -> existing));
  ...
  public static class ServiceCall {
    ...
    public ServiceCall(
        String fromService, String toService, String operation, Long duration, boolean isError) {
  ...
  public static class RpcInterface {
    ...
    public RpcInterface(
        String serviceName, String interfaceName, String protocol, Long duration, boolean isError) {
