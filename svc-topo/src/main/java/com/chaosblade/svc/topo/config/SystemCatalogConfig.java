...
public class SystemCatalogConfig {
    ...
    try {
      ...
      if (resource.exists()) {
        try (InputStream inputStream = resource.getInputStream()) {
          ...
          logger.info(
              "Successfully loaded {} systems from system-catalog.json", systemCatalog.size());
        }
      } else {
  ...
  public JaegerSource jaegerSource() {
    ...
    String effectiveJaegerHost =
        (envJaegerHost != null && !envJaegerHost.isEmpty()) ? envJaegerHost : jaegerHost;
    int effectiveJaegerHttpPort = jaegerHttpPort;
    String effectiveEntryService =
        (envEntryService != null && !envEntryService.isEmpty()) ? envEntryService : entryService;
    String effectiveSystemName =
        System.getenv("SystemName") != null && !System.getenv("SystemName").isEmpty()
    ...
    logger.info(
        "Created JaegerSource with effective config: host={}, httpPort={}, entryService={},"
            + " systemKey={}",
        effectiveJaegerHost,
        effectiveJaegerHttpPort,
        effectiveEntryService,
        effectiveSystemName);
    return jaegerSource;
  }
