...
/**
 * API请求配置类
 * 用于加载和管理system-request.json文件中的ApiRequestPayload数据
 */
@Component
public class ApiRequestConfig {
    ...
    /**
     * 从system-request.json文件加载ApiRequestPayload数据
     * 首先尝试从应用程序根目录加载，如果不存在则从classpath加载
     */
        ...
        try {
            ...
            if (file.exists()) {
                try (InputStream inputStream = java.nio.file.Files.newInputStream(file.toPath())) {
                    ...
                    apiRequestPayloads = objectMapper.readValue(inputStream, new TypeReference<List<ApiRequestPayload>>() {});
                    logger.info("Successfully loaded {} API request payloads from system-request.json in app root", apiRequestPayloads.size());
                    return;
                }
            }
            ...
            if (resource.exists()) {
                try (InputStream inputStream = resource.getInputStream()) {
                    ObjectMapper objectMapper = new ObjectMapper();
                    apiRequestPayloads = objectMapper.readValue(inputStream, new TypeReference<List<ApiRequestPayload>>() {});
                    logger.info("Successfully loaded {} API request payloads from system-request.json in classpath", apiRequestPayloads.size());
                }
            } else {
                logger.warn("system-request.json not found in app root or classpath, initializing with empty list");
