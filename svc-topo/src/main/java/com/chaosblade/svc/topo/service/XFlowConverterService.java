...
public class XFlowConverterService {
  ...
  private List<Map<String, Object>> convertEdges(List<Edge> edges) {
    return edges.stream().map(this::convertEdge).collect(Collectors.toList());
  }
  ...
  public Map<String, Object> getNodeDetails(String nodeId) {
    try {
      ...
      Optional<Node> nodeOpt =
          topology.getNodes().stream().filter(node -> node.getNodeId().equals(nodeId)).findFirst();
  ...
  /** 应用布局算法 */
  public Map<String, Object> applyLayout(
      String algorithm, String direction, Map<String, Object> options) {
  ...
  private Map<String, Object> createStatistics(TopologyGraph topology) {
    ...
    // 节点类型统计
    Map<EntityType, Long> nodeTypeCount =
    ...
    // 边类型统计
    Map<RelationType, Long> edgeTypeCount =
