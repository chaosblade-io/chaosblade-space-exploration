 ...
 * Trace文件上传控制器
 *
 * <p>提供REST API接口： 1. 文件上传和解析 2. 拓扑图生成
 */
@RestController
...
public class TraceUploadController {
  ...
  public ResponseEntity<Map<String, Object>> uploadMultipleTraceFiles(
    ...
    try {
      // 1. 验证文件数量
      if (files.length == 0) {
        return ResponseEntity.badRequest().body(createErrorResponse("未选择文件", "NO_FILES"));
      }
      ...
      for (MultipartFile file : files) {
        ...
        if (!isValidTraceFile(file)) {
          return ResponseEntity.badRequest()
              .body(
                  createErrorResponse(
                      "无效的trace文件格式: " + file.getOriginalFilename(), "INVALID_FORMAT"));
        }
      }
      ...
      logger.info(
          "批量trace文件处理完成: {} 个文件, {} 个服务, {} 个节点, {} 条边",
          traceDataList.size(),
          serviceNames.size(),
          statistics.getNodeCount(),
  ...
  @PostMapping("/upload")
  public ResponseEntity<Map<String, Object>> uploadTraceFile(
      @RequestParam("file") MultipartFile file) {
    logger.info("收到trace文件上传请求: {}, 大小: {} bytes", file.getOriginalFilename(), file.getSize());

    try {
      // 1. 验证文件
      if (file.isEmpty()) {
        return ResponseEntity.badRequest().body(createErrorResponse("文件为空", "EMPTY_FILE"));
      }
  ...
  /** 仅解析trace文件，返回基本信息 */
  @PostMapping("/parse")
  public ResponseEntity<Map<String, Object>> parseTraceFile(
      @RequestParam("file") MultipartFile file) {
    ...
    try {
      ...
      response.put(
          "traceRecordCount", traceData.getData() != null ? traceData.getData().size() : 0);
  ...
  public ResponseEntity<Map<String, Object>> generateTopologyFromJson(
      @RequestBody String traceJson) {
  ...
  public ResponseEntity<Map<String, Object>> validateTraceFile(
      @RequestParam("file") MultipartFile file) {
