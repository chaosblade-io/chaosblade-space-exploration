...
public class ProxyClient {
  ...
  private <T> T retrying(Call<T> call) {
    ...
    while (attempts < Math.max(1, props.getMaxRetries())) {
      ...
      attempts++;
      try {
        Thread.sleep((long) (props.getBackoffMs() * Math.pow(2, attempts - 1)));
  ...
  public Map<String, Object> replay(
      Long executionId, String namespace, String serviceName, Map<String, String> headers) {
    return retrying(
        () -> {
          HttpHeaders h = new HttpHeaders();
  ...
  public Map<String, Object> interceptorsUpsert(
      String namespace, String recordId, int ttlSec, List<Map<String, Object>> items) {
    return retrying(
        () -> {
          Map<String, Object> payload = new LinkedHashMap<>();
          payload.put("namespace", namespace);
