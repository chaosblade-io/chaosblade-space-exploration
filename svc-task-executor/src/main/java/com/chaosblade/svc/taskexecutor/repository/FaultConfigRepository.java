...
public interface FaultConfigRepository extends JpaRepository<FaultConfig, Long> {
  ...
  @Query(
      "SELECT f FROM FaultConfig f WHERE f.nodeId IN :nodeIds AND (f.taskId IS NULL OR f.taskId ="
          + " :taskId)")
  List<FaultConfig> findByNodeIdsWithTaskScope(
      @Param("nodeIds") Collection<Long> nodeIds, @Param("taskId") Long taskId);
}
