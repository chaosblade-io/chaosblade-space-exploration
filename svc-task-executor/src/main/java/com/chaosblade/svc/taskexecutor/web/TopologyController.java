...
public class TopologyController {
    ...
    public TopologyController(TopologyLayerService topologyLayerService,
    ...
    /**
     * 获取按层次分组的叶子节点列表（剥洋葱式层次）。
     * 1) 去掉孤立节点；2) 迭代移除叶子节点，记录每一层。
     */
    ...
    public ApiResponse<List<ServiceFaultConfig>> getFaultConfigs(@PathVariable("taskId") Long taskId) {
     ...
     *
     * - 叶子节点（第1层）: <serviceName, faultConfigId>
     * - 非叶子节点（第2层及以上）: <serviceName, faultConfigId, <downstreamFaultScenarios>>
     * - 基线用例：faultConfigId 为 null 表示不注入故障
     *
     * @param taskId 检测任务ID
     * @return ApiResponse<List<TestCaseDTO>> 返回该任务生成的全部测试用例
     *
     * 错误处理：
     * - 当 taskId 不存在或无对应拓扑时，将抛出 BusinessException，由全局异常处理器统一返回 ApiResponse.error(...)
     */
    ...
    /**
     * Step1：仅输出高价值节点选择结果（打分/覆盖/必测集合），不生成用例。
     * - 对拓扑做 SCC 压缩为 DAG
     * - 计算 in_paths / out_paths 与 score
