...
public class TaskExecuteController {
  ...
  public TaskExecuteController(
      ExecutionOrchestrator orchestrator, ProxyProperties proxyProperties) {
  ...
  public ResponseEntity<ApiResponse<Map<String, Object>>> execute(
      @PathVariable("taskId") Long taskId, @RequestBody(required = false) ExecuteOptions opts) {
    ...
    new Thread(
            () -> {
              ...
              if (opts != null) {
                if (opts.ttlSecForInterceptors != null)
                  options.ttlSecForInterceptors = opts.ttlSecForInterceptors;
                if (opts.waitAnalyzeTimeoutSec != null)
                  options.waitAnalyzeTimeoutSec = opts.waitAnalyzeTimeoutSec;
                if (opts.waitInterceptorReadySec != null)
                  options.waitInterceptorReadySec = opts.waitInterceptorReadySec;
              }
    ...
    return ResponseEntity.ok(
        ApiResponse.ok(Map.of("executionId", executionId, "status", te.getStatus())));
  }

  @GetMapping("/executions/{executionId}")
  public ResponseEntity<ApiResponse<Map<String, Object>>> getExecutionStatus(
