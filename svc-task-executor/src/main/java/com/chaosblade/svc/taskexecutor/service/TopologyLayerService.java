...
public class TopologyLayerService {
    ...
    public TopologyLayerService(DetectionTaskRepository detectionTaskRepository,
    ...
    public List<Layer> getLayersByTaskId(Long taskId) {
        ...
        ApiTopology latest = apiTopologyRepository.findBySystemIdAndApiId(task.getSystemId(), task.getApiId())
                .orElseThrow(() -> new BusinessException("TOPOLOGY_NOT_FOUND", "未找到该系统与API对应的拓扑: system_id="
                        + task.getSystemId() + ", api_id=" + task.getApiId()));
    ...
    private List<Layer> computeLayers(List<ApiTopologyNode> allNodes, List<ApiTopologyEdge> edges) {
        Map<Long, ApiTopologyNode> nodeMap = allNodes.stream()
                .collect(Collectors.toMap(ApiTopologyNode::getId, n -> n));
        ...
        Set<Long> active = nodeMap.keySet().stream()
        ...
        for (Long id : active) {
            ...
            byLevel.computeIfAbsent(lv, k -> new ArrayList<>())
                    .add(new Layer.NodeInfo(id,
                            nodeMap.get(id).getName(),
                            nodeMap.get(id).getNodeKey(),
                            nodeMap.get(id).getProtocol() != null ? nodeMap.get(id).getProtocol().name() : null));
        }
        List<Layer> layers = new ArrayList<>();
        int idx = 1;
        for (Map.Entry<Integer, List<Layer.NodeInfo>> e : byLevel.entrySet()) {
            List<Layer.NodeInfo> nodeInfos = e.getValue().stream()
    ...
    public static class Layer {
        ...
        public static class NodeInfo {
            ...
            public NodeInfo(Long id, String name, String nodeKey, String protocol) {
                this.id = id; this.name = name; this.nodeKey = nodeKey; this.protocol = protocol;
            }
