...
public class KubernetesService implements AutoCloseable {
        ...
        () -> {
          try {
            ...
            List<String> podNames =
                pods.stream().map(p -> p.getMetadata().getName()).distinct().toList();
            ...
            log.warn(
                "K8s query failed for ns={}, svc={}: {}", namespace, serviceName, e.toString());
  ...
  public int getServicePort(String namespace, String serviceName) {
    try {
      ...
      for (var p : ports) {
        if (p.getPort() != null
            && p.getPort() > 0
            && (p.getProtocol() == null || "TCP".equalsIgnoreCase(p.getProtocol()))) {
          return p.getPort();
        }
      }
      // 兜底：第一个定义的端口
      ...
      log.warn(
          "Failed to get service port for ns={}, svc={}: {}", namespace, serviceName, e.toString());
      return -1;
    }
  }
  ...
  private String resolveServiceName(String namespace, String serviceName) {
    try {
      ...
      log.debug(
          "[K8s] resolveServiceName error: ns={}, svc={}, ex={}",
          namespace,
          serviceName,
          e.toString());
    }
  ...
  public boolean waitForServiceStable(String namespace, String serviceName, long timeoutMs) {
    ...
    log.info(
        "[K8s] Waiting service stable: ns={}, svc={}, timeoutMs={}",
        namespace,
        serviceName,
        timeoutMs);
    int lastTotalPods = 0;
    ...
    try {
      while (System.currentTimeMillis() < deadline) {
        ...
        var pods =
            client.pods().inNamespace(namespace).withLabel("app", serviceName).list().getItems();
        if (pods == null || pods.isEmpty()) {
          // 尝试其它 label key
          log.debug("[K8s] No pods found by label 'app', try keys: {}", props.getLabelKeys());
          for (String key : props.getLabelKeys()) {
            pods =
                client.pods().inNamespace(namespace).withLabel(key, serviceName).list().getItems();
            if (pods != null && !pods.isEmpty()) {
                      ...
                      .filter(
                          p -> {
                            try {
                              ...
                              boolean condReady =
                                  st.getConditions() != null
                                      && st.getConditions().stream()
                                          .anyMatch(
                                              c ->
                                                  "Ready".equalsIgnoreCase(c.getType())
                                                      && "True".equalsIgnoreCase(c.getStatus()));
                              boolean containersReady =
                                  st.getContainerStatuses() != null
                                      && st.getContainerStatuses().stream()
        ...
        try {
          ...
          if (resolvedSvcName == null) {
            log.warn(
                "[K8s] No Service found for ns={}, svc={} by name or label keys {}",
                namespace,
                serviceName,
                props.getLabelKeys());
          } else {
            ...
            if (ep != null && ep.getSubsets() != null) {
              epReady =
                  ep.getSubsets().stream()
                      .anyMatch(ss -> ss.getAddresses() != null && !ss.getAddresses().isEmpty());
            } else {
              log.debug(
                  "[K8s] Endpoints object not found or has no subsets for ns={}, svcName={}",
                  namespace,
                  resolvedSvcName);
            }
          }
          ...
          log.debug(
              "[K8s] Endpoints check error for ns={}, svc={}: {}",
              namespace,
              serviceName,
              ex.toString());
        }
        ...
        if (podsReady) {
          log.info(
              "[K8s] Service stable confirmed by Pods readiness: ns={}, svc={}, endpoints={}",
              namespace,
              serviceName,
              epReady);
          return true;
      ...
      log.warn(
          "[K8s] Wait service stable timeout: ns={}, svc={}, lastPodsReady={}/{}, lastEpReady={}",
          namespace,
          serviceName,
          lastReadyCnt,
          lastTotalPods,
      ...
      log.warn(
          "[K8s] waitForServiceStable error for ns={}, svc={}: {}",
          namespace,
          serviceName,
          e.toString());
    }
