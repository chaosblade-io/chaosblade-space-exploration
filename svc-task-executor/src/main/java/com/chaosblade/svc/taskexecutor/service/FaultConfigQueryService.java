...
public class FaultConfigQueryService {
  ...
  public FaultConfigQueryService(
  ...
  public List<ServiceFaultConfig> getFaultConfigsByTaskId(Long taskId) {
    ...
    com.chaosblade.svc.taskexecutor.entity.System system =
        systemRepository
    ...
    List<FaultConfig> configs =
        faultConfigRepository.findByNodeIdsWithTaskScope(nodeIds, task.getId());
    if (configs.isEmpty()) {
      return Collections.emptyList();
    }
    ...
    for (FaultConfig fc : configs) {
      ...
      grouped
          ...
          .add(
              new ServiceFaultConfig.FaultEntry(
                  fc.getId(), fc.getNodeId(), fc.getFaultscript(), fc.getType()));
    }
    ...
    for (Map.Entry<String, List<ServiceFaultConfig.FaultEntry>> e : grouped.entrySet()) {
      ...
      futures.add(
          kubernetesService
              .getDeploymentInfoAsync(namespace, serviceName)
              .thenApply(
                  k ->
                      new ServiceFaultConfig(
                          serviceName,
                          k.getNamespace(),
                          k.getPodNames(),
                          k.getContainerNames(),
