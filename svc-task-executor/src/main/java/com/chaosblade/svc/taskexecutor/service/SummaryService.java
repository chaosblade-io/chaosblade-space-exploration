...
public class SummaryService {
  ...
  public void summarizeAndSave(Long executionId) {
    ...
    List<InterceptReplayResult> rrs =
        interceptReplayResultRepository.findByExecutionId(executionId);
    for (var r : rrs) {
    ...
    String prompt =
        buildPrompt(
            exec, totalCases, failedCases, avgErrRate, affectedServices, allTypes, serviceToTypes);
    String content = null;
    try {
  ...
  private String buildPrompt(
    ...
    if (exec != null) {
      sb.append("- 执行ID: ")
          .append(exec.getId())
          .append("，环境(namespace): ")
          .append(exec.getNamespace())
          .append("，状态: ")
          .append(exec.getStatus())
          .append("\n");
    }
    sb.append("- 用例总数: ")
        .append(totalCases)
        ...
        .append(failedCases)
        .append("，平均错误率(%): ")
        .append(String.format(java.util.Locale.ROOT, "%.2f", avgErrRate))
        .append("\n");
    sb.append("- 受影响服务数: ")
        .append(affectedServices)
        .append("；故障类型覆盖: ")
        .append(allTypes)
        .append("\n");
  ...
  private String fallbackSummary(
      ...
      int affectedServices,
      Set<String> allTypes) {
    return "自动总结暂不可用。基础信息：用例总数="
        + totalCases
        + "，失败用例数="
        + failedCases
        + "，平均错误率(%)="
        + String.format(java.util.Locale.ROOT, "%.2f", avgErrRate)
        + "，受影响服务数="
        + affectedServices
        + "，故障类型="
        + allTypes
