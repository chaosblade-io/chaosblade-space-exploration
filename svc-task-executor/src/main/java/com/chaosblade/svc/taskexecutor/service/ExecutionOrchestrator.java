...
public class ExecutionOrchestrator {
  ...
  private static final org.slf4j.Logger log =
      org.slf4j.LoggerFactory.getLogger(ExecutionOrchestrator.class);
  ...
  public TaskExecution createOrFailIfRunning(Long taskId, boolean force) {
    ...
    DetectionTask task =
        detectionTaskRepository
            .findById(taskId)
            .orElseThrow(
                () -> new BusinessException("DETECTION_TASK_NOT_FOUND", "任务不存在: " + taskId));
    com.chaosblade.svc.taskexecutor.entity.System sys =
        systemRepository
            .findById(task.getSystemId())
            .orElseThrow(
                () -> new BusinessException("SYSTEM_NOT_FOUND", "系统不存在: " + task.getSystemId()));
    TaskExecution te = new TaskExecution();
    te.setTaskId(taskId);
    te.setNamespace(sys.getSystemKey());
    Long reqDefId =
        (task.getApiDefinitionId() != null)
            ? task.getApiDefinitionId().longValue()
            : task.getApiId();
  ...
  public void run(Long executionId, OrchestrateOptions options) {
    TaskExecution te =
        taskExecutionRepository
            ...
            .orElseThrow(
                () -> new BusinessException("TASK_EXECUTION_NOT_FOUND", "执行不存在: " + executionId));
    try {
      taskExecutionLogService.append(
          executionId,
          TaskExecutionLog.LogLevel.INFO,
          "[Orchestrator] Begin execution: status="
              + te.getStatus()
              + ", namespace="
              + te.getNamespace()
              + ", reqDefId="
      ...
      taskExecutionLogService.append(
          executionId, TaskExecutionLog.LogLevel.INFO, "[Stage1] Start analyzing patterns");
      List<String> svcListForAnalyze =
          faultConfigQueryService.getFaultConfigsByTaskId(te.getTaskId()).stream()
              .map(ServiceFaultConfig::getServiceName)
              .distinct()
              .toList();
      ...
      taskExecutionLogService.append(
          executionId,
          TaskExecutionLog.LogLevel.INFO,
          "[Stage2] Analyze request: reqDefId="
              + te.getReqDefId()
              + ", ns="
              + te.getNamespace()
              + ", services="
              + svcListForAnalyze.size()
              + ", durationSec=600, reqCount=1");
      Map<String, Object> analyzeResp =
          proxyClient.analyze(
              new LinkedHashMap<>(
                  Map.of(
                      "reqDefId",
      ...
      String analyzeTaskId =
          asString(((Map<?, ?>) analyzeResp.getOrDefault("data", analyzeResp)).get("taskId"));
      Long recordId =
          asLong(((Map<?, ?>) analyzeResp.getOrDefault("data", analyzeResp)).get("recordId"));
      ...
      taskExecutionLogService.append(
          executionId,
          TaskExecutionLog.LogLevel.INFO,
          "[Stage2] Poll analyze task: taskId="
              + analyzeTaskId
              + ", timeoutSec="
              + options.waitAnalyzeTimeoutSec);
      ...
      if (!"COMPLETED".equalsIgnoreCase(sFinal)) {
        taskExecutionLogService.append(
            executionId, TaskExecutionLog.LogLevel.ERROR, "[Stage2] Analyze timeout");
        throw new BusinessException("ANALYZE_TIMEOUT", "Proxy 分析超时");
      }
      taskExecutionLogService.append(
          executionId, TaskExecutionLog.LogLevel.INFO, "[Stage2] Analyze completed");
      // 阶段2完成后：并行等待所有相关服务恢复稳定（所有 pod 正常）
      try {
        List<String> svcList = svcListForAnalyze;
        ExecutorService wpool =
            Executors.newFixedThreadPool(Math.min(Math.max(1, svcList.size()), 8));
        ...
        for (String svc : svcList) {
          wf.add(
              wpool.submit(
                  () -> {
                    boolean ok =
                        kubernetesService.waitForServiceStable(te.getNamespace(), svc, 120_000L);
        ...
        for (Future<Boolean> f : wf) {
          ...
          try {
            if (Boolean.TRUE.equals(f.get())) okCount++;
          } catch (Exception e) {
            log.warn("[Post-Stage2] Wait task error: {}", e.getMessage());
          }
        }
        taskExecutionLogService.append(
            executionId,
      ...
      taskExecutionRepository.save(te);
      taskExecutionLogService.append(
          executionId, TaskExecutionLog.LogLevel.INFO, "[Stage4] Start injecting & replaying");
      // 收集所有用例中涉及到的唯一服务
      ...
      ExecutorService pool =
          Executors.newFixedThreadPool(Math.min(Math.max(1, services.size()), 8));
      List<Future<Map.Entry<String, String>>> futures =
          new ArrayList<>(); // 返回 <service, bladeName>
      taskExecutionLogService.append(
          executionId,
          TaskExecutionLog.LogLevel.INFO,
                ...
                () -> {
                  try {
                    ...
                    Map<String, Object> full =
                        (Map<String, Object>) (Map<?, ?>) ft.getFaultDefinition();
                    Object specObj = full.get("spec");
                    Map<String, Object> payload =
                        (specObj instanceof Map)
                    ...
                    ResponseEntity<Map<String, Object>> fResp =
                    ...
                    log.info(
                        "[Stage4] Injected fault for service={}, bladeName={}", svc, bladeName);
                    taskExecutionLogService.append(
                        executionId,
                        TaskExecutionLog.LogLevel.INFO,
                    ...
                    if (bladeName != null && !bladeName.isBlank()) {
                      ...
                      while (System.currentTimeMillis() < waitDeadline) {
                        try {
                          ResponseEntity<Map<String, Object>> stResp =
                              restTemplate.exchange(
                          ...
                          if ("Running".equalsIgnoreCase(phase)) {
                            log.info(
                                "[Stage4] Fault is running for service={}, bladeName={}",
                                svc,
                                bladeName);
                            break;
                          }
                    ...
                    Map<String, Object> replay =
                        proxyClient.replay(executionId, te.getNamespace(), svc, headers);
                    ...
                    log.info(
                        "[Stage4] Replay verified for service={}, status={}",
                        svc,
                        rr.getResponseStatus());
                    taskExecutionLogService.append(
                        executionId,
                        TaskExecutionLog.LogLevel.INFO,
                        "[Stage4] Replay verified: service="
                            + svc
                            + ", status="
                            + rr.getResponseStatus());
                    ...
                    log.error(
                        "[Stage4] Fault inject/replay failed for service {}: {}",
                        svc,
                        e.getMessage());
                    taskExecutionLogService.append(
                        executionId,
                        TaskExecutionLog.LogLevel.ERROR,
      ...
      for (Future<Map.Entry<String, String>> f : futures) {
        try {
          Map.Entry<String, String> en = f.get();
          if (en != null) bladeNames.put(en.getKey(), en.getValue());
        } catch (Exception ex) {
          log.error("[Stage4] Future join error: {}", ex.getMessage());
        }
      }
      // 恢复全部故障并等待服务恢复
      for (Map.Entry<String, String> en : bladeNames.entrySet()) {
        String bn = en.getValue();
        String svc = en.getKey();
        try {
          if (bn != null) {
            restTemplate.exchange(
          ...
          log.warn("[Stage4] Recover fault failed for service {}: {}", svc, ex.getMessage());
        }
      }
      // 并行等待所有服务恢复稳定，避免串行阻塞过久
      {
        ...
        for (Future<Boolean> f : wf) {
          try {
            ...
            log.warn("[Stage4] Wait task error: {}", e.getMessage());
          }
        }
      }
      ...
      proxyClient.interceptorsUpsert(
          te.getNamespace(), recordIdStr, options.ttlSecForInterceptors, items);
      ...
      while (System.currentTimeMillis() < readyDeadline) {
        ...
        boolean exists =
            Boolean.TRUE.equals(((Map<?, ?>) st.getOrDefault("data", st)).get("exists"));
        if (exists) break;
        try {
          Thread.sleep(500);
        } catch (InterruptedException ie) {
      ...
      // 不存在则失败
      {
        ...
        if (!exists) {
          taskExecutionLogService.append(
              executionId, TaskExecutionLog.LogLevel.ERROR, "[Stage5] Interceptors not ready");
          throw new BusinessException("INTERCEPTOR_NOT_READY", "拦截器未就绪");
        }
        log.info("[Stage5] Interceptors are ready.");
        taskExecutionLogService.append(
            executionId, TaskExecutionLog.LogLevel.INFO, "[Stage5] Interceptors ready");
      }
      // 阶段5就绪后：并行等待所有目标服务 Pod 再次稳定（拦截器下发可能触发滚动）
      {
              ...
              wpool.submit(
                  () -> {
                    ...
                    if (ok) log.info("[Stage5] Service stable after fixtures upsert: {}", svc);
                      ...
                      log.warn(
                          "[Stage5] Service not stable after fixtures upsert (timeout): {}", svc);
                    return ok;
                  }));
        }
        int okCount = 0;
        ...
        for (Future<Boolean> f : wf) {
          try {
            ...
            log.warn("[Stage5] Wait task error: {}", e.getMessage());
          }
        }
        taskExecutionLogService.append(
            executionId,
      ...
      DetectionTask task =
          detectionTaskRepository
              .findById(te.getTaskId())
              .orElseThrow(
                  () ->
                      new BusinessException(
                          "DETECTION_TASK_NOT_FOUND", "任务不存在: " + te.getTaskId()));
      Long httpReqDefId =
          (task.getApiDefinitionId() != null)
              ? task.getApiDefinitionId().longValue()
              : task.getApiId();
      HttpReqDef def =
          httpReqDefRepository
              .findById(httpReqDefId)
              .orElseThrow(
                  () ->
                      new BusinessException(
                          "HTTP_REQ_DEF_NOT_FOUND", "未找到 http_req_def: id=" + httpReqDefId));
      ...
      log.info(
          "[Stage6] Begin executing all cases. totalCases={}, batchSize={}, perCaseConcurrency={},"
              + " requestsPerCase={}",
          cases.size(),
          batchSize,
          perCaseConcurrency,
      ...
      for (List<EnhancedSimplifiedTestCaseDTO> batch : batches) {
        batchNo++;
        log.info(
            "[Stage6] Executing batch {}/{} ({} cases)", batchNo, batches.size(), batch.size());
        taskExecutionLogService.append(
            executionId,
            TaskExecutionLog.LogLevel.INFO,
        ...
        for (EnhancedSimplifiedTestCaseDTO c : batch) {
          ...
          log.info(
              "[Stage6] Case done: id={}, p50={}, p95={}, p99={}, errRate={}",
              caseId,
              tr.getP50(),
              tr.getP95(),
              tr.getP99(),
      ...
      // 异步触发大模型总结
      try {
        summaryService.summarizeAsync(executionId);
      } catch (Exception ignore) {
      ...
      taskExecutionRepository.save(te);
      taskExecutionLogService.append(
          executionId, TaskExecutionLog.LogLevel.ERROR, "[Run] Uncaught error: " + ex.getMessage());
  ...
  private List<EnhancedSimplifiedTestCaseDTO> generateAllServiceCases(Long taskId) {
    ...
    java.util.function.Function<ServiceFaultConfig, EnhancedFaultTargetDTO> buildOne =
        (sfc) -> {
          String ns = sfc.getNamespace();
          String svc = sfc.getServiceName();
          java.util.List<String> containerValues =
              (sfc.getContainerNames() != null) ? sfc.getContainerNames() : java.util.List.of();
          java.util.List<String> podValues =
              (sfc.getNames() != null) ? sfc.getNames() : java.util.List.of();
    ...
    for (String s : services)
      out.add(new EnhancedSimplifiedTestCaseDTO(java.util.List.of(buildOne.apply(svcMap.get(s)))));
    for (int i = 0; i < services.size(); i++) {
      for (int j = i + 1; j < services.size(); j++) {
        out.add(
            new EnhancedSimplifiedTestCaseDTO(
                java.util.List.of(
  ...
  private String buildCaseId(EnhancedSimplifiedTestCaseDTO c) {
    ...
    List<String> svcs =
        c.getFaults().stream().map(EnhancedFaultTargetDTO::getServiceName).sorted().toList();
  ...
  private String buildBaggageHeader(EnhancedSimplifiedTestCaseDTO c, Long executionId) {
    ...
    List<String> services =
        c.getFaults().stream().map(EnhancedFaultTargetDTO::getServiceName).distinct().toList();
  ...
  private TestResult executeCase(
      HttpReqDef def, String baggageHeader, int requestCount, int concurrency) {
      ...
      futures.add(
          pool.submit(
              () -> {
                ...
                if (def.getBodyMode() == HttpReqDef.BodyMode.JSON
                    && def.getBodyTemplate() != null) {
                  rh.setContentType(MediaType.APPLICATION_JSON);
                  body = def.getBodyTemplate();
                  reqEntity = new HttpEntity<>(body, rh);
                } else if (def.getBodyMode() == HttpReqDef.BodyMode.RAW
                    && def.getRawBody() != null) {
                  MediaType ct =
                      MediaType.parseMediaType(
                          Optional.ofNullable(def.getContentType()).orElse("text/plain"));
                ...
                try {
                  ResponseEntity<String> entity =
                      restTemplate.exchange(
                          java.net.URI.create(requestUrl), method, reqEntity, String.class);
                  statusCode = entity.getStatusCode().value();
                } catch (HttpStatusCodeException ex) {
                  statusCode = ex.getStatusCode().value();
                } catch (Exception ex) {
                  //  e9 9d 9e HTTP
                  //  e7 8a b6 e6 80 81 e7 b1 bb e5 bc 82 e5 b8 b8 e4 b9 9f e8 ae a1 e4 b8 ba e9 94 99 e8 af af
                  statusCode = -1;
                } finally {
                  long durMs =
                      java.util.concurrent.TimeUnit.NANOSECONDS.toMillis(System.nanoTime() - begin);
                  durations.add(durMs);
    ...
    for (Future<Void> f : futures) {
      try {
        ...
        log.warn("[Stage6] Single request failed: {}", ex.getMessage());
      }
    }
    ...
    java.math.BigDecimal errRate =
        new java.math.BigDecimal(errors[0] * 100.0 / Math.max(1, requestCount))
  ...
  @Transactional
  private void writeBaggageMap(
      Long taskId, Long executionId, List<EnhancedSimplifiedTestCaseDTO> cases) {
    ...
    for (ServiceFaultConfig sc : cfgs) {
      java.util.Set<String> types =
          svcTypes.computeIfAbsent(sc.getServiceName(), k -> new LinkedHashSet<>());
  ...
  private List<Map<String, Object>> buildInterceptorItems(Long executionId) {
    ...
    List<InterceptReplayResult> rrs =
        interceptReplayResultRepository.findByExecutionId(executionId).stream().toList();
  ...
  @Transactional
  protected java.util.Map<String, Long> persistGeneratedCases(
      Long taskId, Long executionId, List<EnhancedSimplifiedTestCaseDTO> cases) {
    ...
    for (EnhancedSimplifiedTestCaseDTO c : cases) {
      int cnt = (c.getFaults() == null) ? 0 : c.getFaults().size();
      com.chaosblade.svc.taskexecutor.entity.TestCase tc =
          new com.chaosblade.svc.taskexecutor.entity.TestCase();
      tc.setTaskId(taskId);
      ...
      try {
        String json =
            om.writeValueAsString(c.getFaults() == null ? java.util.List.of() : c.getFaults());
