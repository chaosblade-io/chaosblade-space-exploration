...
public class TestExecutionService {
  ...
  public static class ExecutionResult {
    ...
    public EnhancedSimplifiedTestCaseDTO
        testCase; // 完整用例定义（含 faults/namespace/serviceName/faultDefinition）
            ...
            .orElseThrow(
                () ->
                    new BusinessException(
                        "TOPOLOGY_NOT_FOUND",
                        "未找到拓扑: system=" + task.getSystemId() + ", api=" + task.getApiId()));
  ...
  public PatternAnalysisResult getPatternAnalysis(String executionId) {
    ...
    if (r == null)
      throw new BusinessException("PATTERN_EXECUTION_NOT_FOUND", "分析执行不存在: " + executionId);
    return r;
  }

  public ExecutionResult startExecution(Long taskId) {
    // 1) 选用例（非基线）
    List<EnhancedSimplifiedTestCaseDTO> cases =
        testCaseGenerationService.generateEnhancedSimpleCases(taskId);
    ...
    EnhancedSimplifiedTestCaseDTO picked =
        nonBaseline.get(new Random().nextInt(nonBaseline.size()));
    ...
    try {
      if (def.getHeaders() != null) {
        headers =
            new ObjectMapper()
                .readValue(def.getHeaders(), new TypeReference<Map<String, String>>() {});
      }
    ...
    if (def.getBodyMode() == HttpReqDef.BodyMode.JSON && def.getBodyTemplate() != null) {
      ...
      MediaType ct =
          MediaType.parseMediaType(Optional.ofNullable(def.getContentType()).orElse("text/plain"));
    ...
    try {
      ResponseEntity<String> entity =
          restTemplate.exchange(java.net.URI.create(url), method, reqEntity, String.class);
    ...
    for (String bn : bladeNames) {
      try {
        ResponseEntity<Map<String, Object>> delResp =
            restTemplate.exchange(
    ...
    r.testCase = picked;
    r.targetService =
        picked.getFaults().isEmpty() ? null : picked.getFaults().get(0).getServiceName();
    r.targetServices =
        picked.getFaults().stream().map(EnhancedFaultTargetDTO::getServiceName).toList();
    ...
    r.responseBodySnip =
        (respBody != null ? respBody.substring(0, Math.min(1000, respBody.length())) : null);
  ...
  private java.util.List<java.util.Map<String, Object>> extractRulesFromAnalysis(
      Map<String, Object> analysisResponse, String serviceName) {
  ...
  private String normalizePath(String url) {
    ...
    for (String s : segs) {
      ...
      boolean looksUuid =
          ls.matches("[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}");
                ...
                for (int i = 0; i < cases.size(); i++) {
                  ...
                  if (one != null && one.serviceList != null) {
                    for (String svc : one.serviceList) {
                      ...
                      boolean ok =
                          kubernetesService.waitForServiceStable(one.namespace, svc, 120_000L);
  ...
  private int resolveAppPort(String namespace, String serviceName) {
    ...
    int k8sPort = -1;
    try {
      k8sPort = kubernetesService.getServicePort(namespace, serviceName);
    } catch (Exception ignore) {
    }
  ...
  public CombinedExecutionResult startWithRecording(Long taskId) {
    ...
    EnhancedSimplifiedTestCaseDTO picked =
        nonBaseline.get(new Random().nextInt(nonBaseline.size()));
    java.util.List<String> svcList =
        picked.getFaults().stream()
    ...
    java.util.List<Map<String, Object>> defaultRules =
        java.util.List.of(
    ...
    for (String svcName : cr.serviceList) {
      ...
      java.util.List<java.util.Map<String, Object>> rules =
          extractRulesFromAnalysis(analysisResponse, svcName);
    ...
    for (String svcName : cr.serviceList) {
      try {
        String url =
            String.format(
                proxyProperties.getBaseUrl() + "/api/direct-tap/%s/%s/entries",
                cr.namespace,
                svcName);
        ...
        ResponseEntity<Map<String, Object>> tap =
            restTemplate.exchange(
                url,
                HttpMethod.GET,
                requestEntity,
  ...
  public CombinedExecutionResult getCombined(String executionId) {
    ...
    if (r == null)
      throw new BusinessException("COMBINED_EXECUTION_NOT_FOUND", "综合执行不存在: " + executionId);
    return r;
  }
}
