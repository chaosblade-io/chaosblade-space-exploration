...
public class TestCaseGenerationService {
  ...
  public List<TestCaseDTO> generateForTask(Long taskId) {
    ...
    Map<Long, String> id2name =
        nodes.stream().collect(Collectors.toMap(ApiTopologyNode::getId, ApiTopologyNode::getName));
    Map<Long, Set<Long>> g = new LinkedHashMap<>();
    Map<Long, Set<Long>> rg = new LinkedHashMap<>(); // 反图
    for (ApiTopologyNode n : nodes) {
      g.put(n.getId(), new LinkedHashSet<>());
      rg.put(n.getId(), new LinkedHashSet<>());
    }
    ...
    for (Long id : orderByScore) {
      ...
      if (must.size() >= highScoreTopK || (double) covered / (double) totalPaths >= pathCoverage)
        break;
    }
    ...
    for (ServiceFaultConfig sfc : svcFaults) {
      int replicas =
          (sfc.getNames() != null && !sfc.getNames().isEmpty()) ? sfc.getNames().size() : 1;
    ...
    for (Long id : must) {
      ...
      String reason =
          outDeg.getOrDefault(id, 0) == 0
              ? "LEAF"
              : (outDeg.getOrDefault(id, 0) > 1 ? "BRANCH_POINT" : "HIGH_SCORE");
      if (replicas <= 1) {
        focused.add(
            new TestCaseDTO(
                svc,
  ...
  public static class Step1Result {
    public static class NodeScore {
      ...
      private long score;
      private long inPaths;
      private long outPaths;
      ...
      public long getInPaths() {
        return inPaths;
      }

      public long getOutPaths() {
        return outPaths;
      }
    ...
    public Step1Result(
        List<NodeScore> selected,
        List<NodeScore> ranked,
        long totalPaths,
        long coveredPaths,
        int topK,
    ...
    public List<NodeScore> getSelected() {
      return selected;
    }

    public List<NodeScore> getRanked() {
      return ranked;
    }
  ...
  public Step1Result computeStep1(Long taskId) {
    ...
    List<ApiTopologyEdge> edges = edgeRepository.findByTopologyId(topo.getId());
    if (nodes.isEmpty())
      return new Step1Result(List.of(), List.of(), 0, 0, highScoreTopK, pathCoverage);
    ...
    long totalPaths = 0;
    for (int v = 0; v < compN; v++) if (!hasOut[v]) totalPaths += inPaths[v];
    if (totalPaths == 0) totalPaths = nodes.size();
    ...
    List<Step1Result.NodeScore> ranked =
        score.entrySet().stream()
            .sorted((a, b) -> Long.compare(b.getValue(), a.getValue()))
            .map(
                en ->
                    new Step1Result.NodeScore(
                        en.getKey(),
                        id2name.get(en.getKey()),
                        en.getValue(),
                        inPaths[comp.get(en.getKey())],
                        outPaths[comp.get(en.getKey())],
                        outDeg.getOrDefault(en.getKey(), 0),
                        null,
                        comp.get(en.getKey())))
            .toList();
    ...
    for (Step1Result.NodeScore ns : ranked) {
      ...
      int c = ns.getCompId();
      if (picked.add(c)) {
        must.add(ns.getNodeId());
        covered += ns.getInPaths() * ns.getOutPaths();
      }
      if (must.size() >= highScoreTopK || (double) covered / (double) totalPaths >= pathCoverage)
        break;
    }
    ...
    for (Step1Result.NodeScore ns : ranked) {
      ...
      String reason =
          (outDeg.getOrDefault(ns.getNodeId(), 0) == 0)
              ? "LEAF"
              : (outDeg.getOrDefault(ns.getNodeId(), 0) > 1 ? "BRANCH_POINT" : "HIGH_SCORE");
      selected.add(
          new Step1Result.NodeScore(
              ns.getNodeId(),
              ns.getServiceName(),
  ...
  public List<SimplifiedTestCaseDTO> generateSimplifiedForTask(Long taskId) {
    ...
    TestCaseDTO.ChaosSpec allDown =
        new TestCaseDTO.ChaosSpec("all", null, 60, 60, "FAST_5XX_OR_TIMEOUT");
    TestCaseDTO.ChaosSpec oneDown =
        new TestCaseDTO.ChaosSpec(null, 1, 60, 60, "OK_200_OR_AFTER_RETRY");
    ...
    for (Step1Result.NodeScore ns : targets) {
      ...
      var ft =
          new SimplifiedTestCaseDTO.FaultTarget(
              svc,
              chaos,
              replicas,
              ns.getReason() != null ? ns.getReason() : "HIGH_SCORE",
              ns.getScore());
    ...
    for (int i = 0; i + 1 < targets.size(); i += 2) {
      ...
      String svA = a.getServiceName();
      String svB = b.getServiceName();
      int repA = svcReplicas.getOrDefault(svA, 1);
      int repB = svcReplicas.getOrDefault(svB, 1);
      ...
      var fA =
          new SimplifiedTestCaseDTO.FaultTarget(
              ...
              chaosA,
              repA,
              a.getReason() != null ? a.getReason() : "HIGH_SCORE",
              a.getScore());
      var fB =
          new SimplifiedTestCaseDTO.FaultTarget(
              svB,
              chaosB,
              repB,
              b.getReason() != null ? b.getReason() : "HIGH_SCORE",
              b.getScore());
  ...
  public List<MinimalSimplifiedTestCaseDTO> generateMinimalSimplifiedForTask(Long taskId) {
    ...
    for (Step1Result.NodeScore ns : targets) {
      ...
      singles.add(
          new MinimalSimplifiedTestCaseDTO(
              List.of(new MinimalSimplifiedTestCaseDTO.Fault(ns.getServiceName(), fid))));
    }
    ...
    for (int i = 0; i + 1 < targets.size(); i += 1) {
      var a = targets.get(i);
      var b = targets.get(i + 1);
      Long fa = pickFaultId.get(a.getServiceName());
      Long fb = pickFaultId.get(b.getServiceName());
      ...
      for (int j = i + 1; j < targets.size(); j++) {
        ...
        duals.add(
            new EnhancedSimplifiedTestCaseDTO(
                java.util.List.of(buildOne.apply(a), buildOne.apply(b))));
      }
    }
